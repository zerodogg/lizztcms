...
{
    $term = Term::ReadLine->new('lizztcmsinstall');
    my $prevStatus = undef;

    newStep('Welcome');
    print "Welcome to the LizztCMS installer. This script will help you create\n";
    print "the initial LizztCMS database along with the initial admin user, as well\n";
    ...
    print "lizztcms_upgrade.pl script if you are upgrading an existing install.\n\n";
    ...
    print "LizztCMS data directories. Normal configuration simply lets you set a\n";
    print "primary path for the container of the data directory, then LizztCMS will\n";
    ...
    else
    {
        ...
        $settings{installPath} = getPath('Where do you want to install LizztCMS?','Are you sure you want to install LizztCMS there anyway?');
    }

    if ( -e $settings{installPath}.'/lizztcms.yml')
    {
        die($settings{installPath}.'/lizztcms.yml: exists. Use lizztcms_upgrade.pl if you want to upgrade an existing install.'."\n");
    }
    ...
    if($useAdv)
    {
        ...
        while(1)
        {
            print "The LizztCMS file directory must be outside of LizztCMS' install path\n";
            $settings{file_path} = getPath('Where should LizztCMS save its files?');
        ...
        while(1)
        {
            print "The LizztCMS template directory must be outside of LizztCMS' install path\n";
            $settings{template_path} = getPath('Where should LizztCMS save its templates?');
        ...
        while(1)
        {
            print "The LizztCMS temporary directory must be outside of LizztCMS' install path\n";
            $settings{temp_path} = getPath('Where should LizztCMS save its temporary files?');
        ...
        while(1)
        {
            print "The LizztCMS indexer directory must be outside of LizztCMS' install path\n";
            $settings{indexFiles} = getPath('Where should LizztCMS save its indexer files?');
    ...
    else
    {
        ...
        print "The LizztCMS data directory must be outside of LizztCMS' install path\n";
        my $coreData;
        my $default = '/var/lizztcms.data/'.basename($settings{installPath});
        while(1)
        {
            $coreData = getPath('Where should LizztCMS save its data?',undef,$default);
            $coreData .= '/';
            $coreData =~ s{/+}{/}g;
            my $instPath = $settings{installPath}.'/';
            $instPath =~ s{/+}{/}g;
    ...
    print "LizztCMS will create at least one initial administrative user that you can use\n";
    print "to log in to LizztCMS\n";
    addUser();
    ...
    {
        $defaultEmail = 'lizztcms@'.$hostname;
    }

    newStep('From e-mail');
    print "LizztCMS regulary sends e-mails to users for various reasons.\n";
    my $email = getInput('What do you want to be the From: e-mail in emails sent by LizztCMS?',$defaultEmail);
    $settings{from_email} = $email;

    my $dbsuccess = 0;
    newStep('Database configuration');
    print "WARNING: If there is any LizztCMS data already in the database\n";
    ...
    while(1)
    {
        $dbnam = getInput('What is the name of the database LizztCMS should use?',$dbnam);
        $dbuser //= $dbnam;
    ...
    print "there are no conflicts between LizztCMS instances. A namespace MUST therefore\n";
    ...
    newStep('Summary');
    print "LizztCMS is now ready to be installed. Please check that the following settings\n";
    print "are correct.\n";
    my $fmt = '%-32s : %s'."\n";
...
print "Configuring...";
createConfigFile();
print "done - wrote $settings{installPath}/lizztcms.yml\n";
print "Creating database (might take a little while)...";
mysql_exec('source '.$settings{installPath}.'/sql/lizztcms_schema.sql',true);
print "done\n";
...
if ($@)
{
    die("failed. LizztCMS could not initialize. Perhaps some module is missing?\n\nCrash dump:\n-----\n$@\n-----\n");
}
print "done\n";
print "Creating action settings...";
my $i18n = LizztCMS::HelperModules::I18N->new();
my %actionPaths = LizztCMS::Schema::LzAction->getPathsHash($i18n);
foreach my $path (keys(%actionPaths))
{
    my $s = LizztCMS->model('LizztCMSDB::LzAction')->find({ action_path => $path });
    if ($s)
    {
        next;
    }
    print '.';
    $s = LizztCMS->model('LizztCMSDB::LzAction')->create({ action_path => $path });
    $s->update();
}
print "done\n";
...
{
    my $s = LizztCMS->model('LizztCMSDB::LzStatus')->create({
            status_id => $n,
            status_name => $_,
            system_status => '1',
        });
...
my $role = LizztCMS->model('LizztCMSDB::LzRole')->create({
        role_name => 'Admin',
        role_status => 'Active',
    });
print '.';
my $actions = LizztCMS->model('LizztCMSDB::LzAction');
while(my $a = $actions->next)
{
    LizztCMS->model('LizztCMSDB::LzRoleAction')->create({
...
{
    print '.';
    my $user = LizztCMS->model('LizztCMSDB::LzUser')->create({
            created => \'now()',
            firstname => 'changeme',
            lastname => 'changeme',
...
LizztCMS->model('LizztCMSDB::LzFileClass')->create({
        id => 0,
...
{
    print "\n";
    print "LizztCMS has now been successfully installed.\n";
...
sub createField
{
    ...
    my $newField = LizztCMS->model('LizztCMSDB::LzField')->create({
            field_name => $name,
            field_type => $type,
            field_height => $height,
            inline => $inline,
...
{
    open(my $template,'<',$settings{installPath}.'/lizztcms.yml.tpl');
    open(my $target,'>',$settings{installPath}.'/lizztcms.yml');
    my $ses = $settings{temp_path}.'/lizztcmsSessionStorage';
    my $mason = $settings{temp_path}.'/mason/';
    my $root = $settings{installPath}.'/root/';
    while(my $l = <$template>)
    {
        next if $l =~ /lizztcms_install\.pl/;
        if(length($l))
        {
            $l =~ s/\[DBNAME\]/$dbnam/;
            $l =~ s/\[DBUSER\]/$dbuser/;
    ...
    if (!$bootstrap)
    {
        unlink($settings{installPath}.'/lizztcms.yml.tpl');
    }
}
...
sub newStep
{
    ...
    if ($totalSteps > 0)
    {
        $step = 'LizztCMS installer [Step '.$stepNo.' of '.$totalSteps.']'.$stepName;
    }
    else
    {
        $step = 'LizztCMS installer'.$stepName;
    }
    for(0..length($step))
    {
        print '*';
