...
system('perl','script/lizztcms_create.pl','model','DBIC','DBIC::Schema','LizztCMS::Schema','create=static',$conf->{'Model::LizztCMSDB'}->{'connect_info'}->{'dsn'},$conf->{'Model::LizztCMSDB'}->{'connect_info'}->{'user'},$conf->{'Model::LizztCMSDB'}->{'connect_info'}->{'password'});
unlink('./t/model_DBIC.t','./lib/LizztCMS/Model/DBIC.pm','lib/LizztCMS/Schema/LzDmMap.pm');
...
foreach my $file (glob('./lib/LizztCMS/Schema/*.pm'))
{
    checkForChanges($file);
}
checkForChanges('lib/LizztCMS/Schema.pm');
if (@cleanThis)
{
    if ($SCM eq 'git')
    {
