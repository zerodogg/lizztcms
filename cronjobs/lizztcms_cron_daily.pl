...
my $title = 'LizztCMS_Daily_Cron ['.getcwd.']';
...
# LizztCMS initialization (loads dependencies etc.)
sub initialize
{
    title('init','Initializing the LizztCMS core...');
    ...
    silent
    {
        eval('use LizztCMS::HelperModules::Scripts qw(fakeC); use LizztCMS::HelperModules::Indexer;1;') or die($@);
    };

    $fakeC  = fakeC();

    if(ref($fakeC->config->{'Model::LizztCMSDB'}->{'connect_info'}) eq 'ARRAY')
    {
        die("Old-style config. Unable to continue.\n");
    }
    my $DBSTR  = $fakeC->config->{'Model::LizztCMSDB'}->{'connect_info'}->{dsn};
    my $DBUser = $fakeC->config->{'Model::LizztCMSDB'}->{'connect_info'}->{user};
    my $DBPwd  = $fakeC->config->{'Model::LizztCMSDB'}->{'connect_info'}->{password};
...
sub cleanImgcache
{
    ...
    my $path = $fakeC->config->{LizztCMS}->{file_path};
...
sub fixMultipleDefaultTemplates
{
    ...
    my $defaultTemplates = $fakeC->model('LizztCMSDB::LzTemplate')->search({ is_default => 1 });
    my %seenMap;
...
sub addMissingLzAction
{
	...
	my $i18n = LizztCMS::HelperModules::I18N->new('lizztcms','en_US',$fakeC->path_to('i18n','locale')->stringify);
	my %actionPaths = LizztCMS::Schema::LzAction->getPathsHash($i18n);
	foreach my $k (keys %actionPaths)
	{
		$fakeC->model('LizztCMSDB::LzAction')->find_or_create({ action_path => $k });
	}
}
...
sub fixArticleIssues
{
    ...
    my %seen;
    my $folders = $fakeC->model('LizztCMSDB::LzArticleFolder')->search({ primary_folder => 1});
...
{
    my $categoryLayouts = $fakeC->model('LizztCMSDB::LzCategoryLayout');
...
sub updateIndex
{
	if (!$skipIndex)
	{
		...
		my $internalIndexer = LizztCMS::HelperModules::Indexer->new(config => $fakeC->config->{'LizztCMS'}->{'indexer'}, mode => 'internal', c => $fakeC, reindex => $reIndex);
		my $liveIndexer = LizztCMS::HelperModules::Indexer->new(config => $fakeC->config->{'LizztCMS'}->{'indexer'}, mode => 'external', c => $fakeC, reindex => $reIndex);
        ...
        tryRun
        {
            ...
            my $allArts = $fakeC->model('LizztCMSDB::LzArticle')->page(1);
        ...
        tryRun
        {
            ...
            my $allFiles = $fakeC->model('LizztCMSDB::LzFile')->page(1);
...
sub performExpensiveArticleFixes
{
    ...
    my $arts = $fakeC->model('LizztCMSDB::LzArticle');
    while(my $art = $arts->next)
    {
        # Articles without workflows will be sort of lost within the system
        if(not $art->workflow)
        {
            my $wf = $fakeC->model('LizztCMSDB::LzWorkflow')->create({
        ...
        if(not $art->revisionMeta)
        {
            ...
            try
            {
                my $latest = $fakeC->model('LizztCMSDB::LzArticle')->search({ article_id => $art->article_id }, { order_by => 'publish_time DESC' });
                if ($latest->first->revision == $art->revision)
                {
                    $isLatest = 1;
                }
            };
            my $rm = $fakeC->model('LizztCMSDB::LzRevision')->create({
                    type => 'article',
        ...
        if(not $art->primary_folder)
        {
            ...
            else
            {
                ...
                try
                {
                    $f = $fakeC->model('LizztCMSDB::LzFolder')->search(undef, { order_by => 'folder_id ASC' })->first;
                    my $newpr = $fakeC->model('LizztCMSDB::LzArticleFolder')->create({
                            article_id => $art->article_id,
                            revision => $art->revision,
...
sub fixArticleStatusIssues
{
    ...
    my $statuses = $fakeC->model('LizztCMSDB::LzStatus')->search();
    ...
    my %processed;
    my $articles = $fakeC->model('LizztCMSDB::LzArticle')->search({
    ...
    while(my $article = $articles->next)
    {
        ...
        if (!$article->revisionMeta->is_latest_exclusive_status)
        {
            ...
            $fakeC->model('LizztCMSDB::LzRevision')->search({
                    type => 'article',
    ...
    foreach my $artid (keys %processed)
    {
        ...
        while(my $status = $statuses->next)
        {
            my $articlesInStatus = $fakeC->model('LizztCMSDB::LzArticle')->search({
                    article_id => $artid,
...
sub performTreeRecursionChecks
{
    ...
    {
        my $objects = $fakeC->model('LizztCMSDB::'.$type);
...
sub dropBrokenMetaObjects
{
    ...
    {
        my $object = $fakeC->model('LizztCMSDB::'.$type);
        while(my $obj = $object->next)
        {
            ...
            my $art = $fakeC->model('LizztCMSDB::LzArticle')->find($search);
            if (!$art)
            {
    ...
    foreach my $type (qw(LzArticleFile LzFileFolder LzFileTag))
    {
        ...
        while(my $obj = $object->next)
        {
            my $file = $fakeC->model('LizztCMSDB::LzFile')->find({
                    file_id => $obj->file_id
                });
            if (!$file)
            {
    ...
    my $object = $fakeC->model('LizztCMSDB::LzNewsletterSubscriptionGroup');
    while(my $obj = $object->next)
    {
        my $group = $fakeC->model('LizztCMSDB::LzNewsletterGroup')->find({
