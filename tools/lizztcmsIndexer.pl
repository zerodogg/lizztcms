...
# Simple helper script for indexing in LizztCMS.
#
...
# the LizztCMS core (but uses a mock from HelperModules::Scripts)
# and leaves index optimization to the indexer cronjob.
# It also loads most time-consuming modules after checking if
# --fork was supplied, so that it does not block LizztCMS from completing
# a request.
use strict;
use warnings;
use Getopt::Long;
...
GetOptions(
    ...
    'delete' => \$delete,
    'help' => sub {
        print "Indexing helper for LizztCMS. Not meant for direct execution\n";
        exit(0);
    },
) or die;
...
nice(10); # Other processes (read: LizztCMS core) are more important
eval('use LizztCMS::HelperModules::Scripts qw(getConfig mockC);
use LizztCMS::HelperModules::Indexer;
use LizztCMS::Schema;1;') or die;
...
my $row;
my $rsName;
my $schema = LizztCMS::Schema->connect($config->{'Model::LizztCMSDB'}->{connect_info});;
my $indexer;

if ($type eq 'article')
{
    $indexer = LizztCMS::HelperModules::Indexer->new(config => $config->{'LizztCMS'}->{'indexer'}, mode => 'external', c => $c);
    $rsName = 'LzArticle';
}
elsif($type eq 'file')
{
    $indexer = LizztCMS::HelperModules::Indexer->new(config => $config->{'LizztCMS'}->{'indexer'}, mode => 'internal', c => $c);
    $rsName = 'LzFile';
}
