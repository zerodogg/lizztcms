...
=cut
package LizztCMS::Controller::Live::Newsletter;
...
use LizztCMS::HelperModules::Live::CAPTCHA qw(validate_captcha);
...
sub subscribe : Local
{
    ...
    my $existing = $c->model('LizztCMSDB::LzNewsletterSubscription')->search({ email => $email });
    ...
    my $new = $c->model('LizztCMSDB::LzNewsletterSubscription')->create({
            email => $email,
            name => $name,
            format => $format,
    ...
    if(defined $language)
    {
        ...
        foreach(@lang)
        {
            if (/^\d+$/ && $c->model('LizztCMSDB::LzNewsletterGroup')->find({ group_id => $_ }))
            {
                $c->model('LizztCMSDB::LzNewsletterSubscriptionGroup')->create({ 
                        group_id => $_,
...
sub unsubscribe : Local
{
    ...
    my $entry = $c->model('LizztCMSDB::LzNewsletterSubscription')->find({
            subscription_id => $uid,
        });
