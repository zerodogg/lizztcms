...
package LizztCMS::Controller::Admin::Settings::Admin::Additionalfields;
...
{
    my ( $self, $c, $query ) = @_;
    my $fields = $c->model('LizztCMSDB::LzField');
...
sub edit : Local Args
{
    ...
    my $populate = {};
    my $field = $c->model('LizztCMSDB::LzField')->find({field_id => $uid});
    if(not $field)
    {
...
sub fieldeditor : Local Args
{
    ...
    {
        $rev //= 0;
        my $art = $c->model('LizztCMSDB::LzArticle')->find({article_id => $uid, revision => $rev});
        if(not $art)
        {
    ...
    {
        my $f = $c->model('LizztCMSDB::LzFolder')->find({folder_id => $uid});
        if(not $f)
        {
    ...
    {
        my $class = $c->model('LizztCMSDB::LzFileClass')->find({ id => $object_id });
        if(not $class)
        {
    ...
    {
        $fields = LizztCMS::HelperModules::Fields->new($c,$module,$uid, { folder_id => $folder_id });
    }
    else
    {
        $fields = LizztCMS::HelperModules::Fields->new($c,$module,$uid, { object_id => $object_id });
    }
    ...
    my $allFields = $c->model('LizztCMSDB::LzField')->search(undef, {order_by => 'field_name,field_id'});
    my $v = 0;
    my $entries = {};
...
{
    my ( $self, $c, $uid ) = @_;
    my $field = $c->model('LizztCMSDB::LzField')->find({field_id => $uid},{prefetch => 'modules'});
    if(not $field)
    {
...
sub submit : Local
{
    ...
    {
        $field = $c->model('LizztCMSDB::LzField')->find({field_id => $r->param("uid")});
    }
    else
    {
        $field = $c->model('LizztCMSDB::LzField')->create({ });
    ...
    elsif($r->param('values'))
    {
        ...
        {
            $val =~ s/^\s//;
            my $o = $c->model('LizztCMSDB::LzFieldOptions')->find_or_create({
...
sub fieldModuleUpdate : Local
{
    ...
    {
        $revision //= 0;
        my $art = $c->model('LizztCMSDB::LzArticle')->find({revision => $revision, article_id => $module_id},{prefetch => 'folders'});
        if(not $art)
        {
    ...
    my $fieldObj = LizztCMS::HelperModules::Fields->new($c,$module_name,$module_id, { folder_id => $folder_id });
