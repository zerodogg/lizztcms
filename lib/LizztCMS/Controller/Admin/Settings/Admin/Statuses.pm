...
package LizztCMS::Controller::Admin::Settings::Admin::Statuses;
...
use LizztCMS::HelperModules::Forms qw(get_checkboxes);
...
{
    my ( $self, $c ) = @_;
    my $fields = $c->model('LizztCMSDB::LzStatus');
...
sub delete : Local Args
{
    ...
    my $status = $c->model('LizztCMSDB::LzStatus')->find({status_id => $uid});
    if (not $status)
    {
        return $self->messageToList($c,$c->stash->{i18n}->get('Status id did not exist.'));
    }
    my $aclEntry = $c->model('LizztCMSDB::LzAction')->find({action_path => 'STATUSCHANGE_'.$status->status_id});
    if(not $aclEntry)
    {
...
sub savedata : Private
{
    ...
    if ($type eq 'edit')
    {
        $status = $c->model('LizztCMSDB::LzStatus')->find({status_id => $uid});
    }
    elsif($type eq 'add')
    {
        $status = $c->model('LizztCMSDB::LzStatus')->create(
            {
            }
        );
        my $aclEntry = $c->model('LizztCMSDB::LzAction')->create({action_path => 'STATUSCHANGE_'.$status->status_id});
        if(not $aclEntry)
        {
