...
package LizztCMS::Controller::Admin::Articles::Workflow;
...
sub submitComment : Local
{
    ...
    if (defined $revision)
    {
        $article = $c->model('LizztCMSDB::LzArticle')->find({article_id => $artid, revision => $revision});
    }
    else
    {
	...
	my $comment = $c->model('LizztCMSDB::LzWorkflowComments')->create({
...
sub comments : Local Arg
{
    ...
    my ( $self, $c, $uid) = @_;
	my $article = $c->model('LizztCMSDB::LzArticle')->search({article_id => $uid})->first();
...
sub preparePage : Private
{
	...
	if ($article)
	{
        my $comments = $c->model('LizztCMSDB::LzWorkflowComments')->search({ article_id => $article->article_id }, { order_by => 'written_time' });
        my $workflow = $article->workflow;
...
sub prepare : Private
{
	...
	if ($workflow)
	{
        ...
        if ($article)
        {
            $c->stash->{w_watched} = $c->model('LizztCMSDB::LzArticleWatch')->find({ user_id => $c->stash->{user_id}, article_id => $article->article_id});
        }
        ...
        {
            my $role = $c->model('LizztCMSDB::LzRole')->find({role_id => $workflow->assigned_to_role});
            if ($role)
            {
        ...
        { 
            my $user = $c->model('LizztCMSDB::LzUser')->find({user_id => $workflow->assigned_to_user});
            if ($user)
            {
        ...
        if (defined $assigned_by and my $user = $c->model('LizztCMSDB::LzUser')->find({user_id => $assigned_by}))
        {
    ...
    if ($c->stash->{w_can_reassign})
    {
        ...
        my $roleList = $c->model('LizztCMSDB::LzRole')->search(undef,{ order_by => 'role_name' });
        ...
        if (@roles)
        {
            ...
            my $userList = $c->model('LizztCMSDB::LzUser')->search(undef,{ order_by => 'user_name' });
...
sub notifyAboutComment
{
    ...
    my $i18n = $c->stash->{i18n};
    my $article = $c->model('LizztCMSDB::LzArticle')->find({article_id => $artid},{columns => ['title'], prefetch => [ 'workflow' ]});
    my $workflow = $article->workflow;
    my $watchers = $c->model('LizztCMSDB::LzArticleWatch')->search({ article_id => $artid },{prefetch => ['user']});
    ...
    my $autoMessage = $i18n->get("This message has been automatically generated by LizztCMS because you\nare either watching this article, or you are the assignee.\n");
