...
package LizztCMS::Controller::Admin::Articles::Submit;
use Moose;
...
use LizztCMS::HelperModules::Calendar qw(create_calendar datetime_to_SQL datetime_from_SQL);
use List::MoreUtils qw(any);
use LizztCMS::HelperModules::RevisionControl::Article;
use LizztCMS::HelperModules::RevisionHelpers qw(article_latest_revisions get_latest_article);
use LizztCMS::HelperModules::HTMLFilter qw(filter_string);
use LizztCMS::HelperModules::EMail qw(send_email_to);
...
sub index_POST
{
    ...
    if ($type eq 'edit')
    {
        ...
        $workflow = $c->model('LizztCMSDB::LzWorkflow')->find_or_create({ article_id => $uid, revision => $article->revision });
    ...
    {
        $article = $c->model('LizztCMSDB::LzArticle')->new_result(
            {
                created_time => \'now()',
            }
        );
        $workflow = $c->model('LizztCMSDB::LzWorkflow')->new_result({ article_id => $article->article_id });
    ...
    my $RCS = LizztCMS::HelperModules::RevisionControl::Article->new( committer => $c->user->user_id );
    ...
    {
        $c->forward(qw(LizztCMS::Controller::Admin::Articles::Workflow notifyRoleMembers), [ $workflow, $c->model('LizztCMSDB::LzRole')->find({ role_id => $workflow->get_column('assigned_to_role') }) ]);
    }
...
sub savedata_relationships
{
    ...
    foreach my $relID (keys %{$submitted})
    {
        ...
        my $newRelationship = $c->model('LizztCMSDB::LzArticleRelations')->new_result({article_id => $article->article_id, related_article_id => $related_article->article_id, relation_type => $submitted->{$relID} });
...
sub savedata_article
{
    ...
    if(defined($c->req->data->{'secondaryFolders'}))
    {
        if ($c->req->data->{'secondaryFolders'} ne 'null')
        {
            ...
            {
                my $art = $c->model('LizztCMSDB::LzArticleFolder')->new_result({
    ...
    if ($c->req->data->{'LZ_ArticleMoveFilesFound'} and $origFolder != $newPrimaryFolder)
    {
        ...
        while((defined $files) && (my $f = $files->next))
        {
            ...
            if ((not defined $file->folder_id) or ($file->folder_id eq $origFolder))
            {
                ...
                {
                    $primary = $c->model('LizztCMSDB::LzFileFolder')->create({
    ...
    if ($type eq 'add')
    {
        $backup = $c->model('LizztCMSDB::LzBackup')->find({user_id => $c->user->user_id, backup_source => 'article', backup_source_id => \'IS NULL'});
    }
    else
    {
        $backup = $c->model('LizztCMSDB::LzBackup')->find({user_id => $c->user->user_id, backup_source => 'article', backup_source_id => $article->article_id});
    }
    if ($backup)
    {
        $backup->delete();
...
sub savedata_files
{
    ...
    {
        my $rel = $RCS->find_or_new('LizztCMSDB::LzArticleFile',{
                article_id => $article->article_id,
...
sub savedata_elements
{
    ...
    {
        my $rel = $RCS->find_or_new('LizztCMSDB::LzArticleElements',{
                article_id => $article->article_id,
                keyvalue_id => $entry,
            });
        $RCS->add_object($rel);
...
sub savedata_tags
{
    ...
    foreach my $entry (@{$submitted})
    {
        my $rel = $RCS->find_or_new('LizztCMSDB::LzArticleTag',{
                article_id => $article->article_id,
                tag_id => $entry,
            });
        $RCS->add_object($rel);
...
sub savedata_workflow
{
    ...
    foreach my $part (keys %{$reassign})
    {
        ...
        elsif(not $reassign->{$part} eq 'null')
        {
            ...
            if ($part eq 'user')
            {
                ...
                {
                    $obj = $c->model('LizztCMSDB::LzUser')->find({ user_id => $reassign->{$part} });
                }
                else
                {
            ...
            elsif ($part eq 'role')
            {
                ...
                {
                    $obj = $c->model('LizztCMSDB::LzRole')->find({ role_id => $reassign->{$part} });
                }
                else
                {
                    return json_error($c,'REASSIGN','Permission denied');
    ...
    $c->forward('LizztCMS::Controller::Admin::Articles::Workflow','prepare', [undef,$workflow]);

    my $watched = $c->model('LizztCMSDB::LzArticleWatch')->find({ user_id => $c->stash->{user_id}, article_id => $article->article_id });
    ...
    {
        $watched = $c->model('LizztCMSDB::LzArticleWatch')->create({ user_id => $c->stash->{user_id}, article_id => $article->article_id });
        $watched->update();
    }
...
sub notifyNewAssignee
{
    ...
    my $assignedTo = $c->model('LizztCMSDB::LzUser')->find({ user_id => $workflow->get_column('assigned_to_user')});
...
sub notifyWatchers : Private
{
    ...
    my $i18n = $c->stash->{i18n};
    my $watchers = $c->model('LizztCMSDB::LzArticleWatch')->search({ article_id => $artid });
    my $hasContent = false;
    if (not $watchers)
    {
        return;
    ...
    if ($artDiff)
    {
        foreach my $key (sort keys %{$artDiff})
        {
            ...
            elsif($key eq 'template_id')
            {
                ...
                {
                    $template = $c->model('LizztCMSDB::LzTemplate')->find({ template_id => $artDiff->{$key}->{$k} });
                    if ($template)
                    {
            ...
            elsif($key eq 'status_id')
            {
                ...
                foreach my $k (qw(old new))
                {
                    $status = $c->model('LizztCMSDB::LzStatus')->find({ status_id => $artDiff->{$key}->{$k} });
                    if ($status)
                    {
...
sub art_save_fielddata
{
    ...
    if(defined $value)
    {
        ...
        elsif($fnam eq 'folder')
        {
            ...
            $f = $c->model('LizztCMSDB::LzArticleFolder')->new_result({ folder_id => $value, article_id => $obj->article_id, primary_folder => 1});
            $RCS->add_object($f);
            $c->stash->{newPrimaryFolder} = $value;
        }
        elsif($fnam eq 'status_id')
        {
            my $status = $c->model('LizztCMSDB::LzStatus')->find({status_id => $value});
            if ($status)
            {
