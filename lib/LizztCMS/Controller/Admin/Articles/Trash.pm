...
package LizztCMS::Controller::Admin::Articles::Trash;

use strict;
use warnings;
use base qw(Catalyst::Controller::FormBuilder);
use LizztCMS::HelperModules::Includes qw(add_jsIncl add_cssIncl add_jsOnLoad add_globalJSVar);
use LizztCMS::HelperModules::JSON qw(json_response json_error);
use LizztCMS::HelperModules::RevisionHelpers qw(get_latest_article article_latest_revisions);
...
sub index : Path Args(0) Form('/core/search')
{
    ...
    my $article = article_latest_revisions($c->model('LizztCMSDB::LzArticle')->search({ trashed => 1 }));
    my $list = $c->forward(qw(LizztCMS::Controller::Admin::Articles retrieveArticles),[ $article, $query, $self->formbuilder,1]);
...
sub delete : Local Args
{
    ...
    else
    {
        ...
        my $backups = $c->model('LizztCMSDB::LzBackup')->search({ backup_source_id => $artid, backup_source => 'article'});
        while(( defined $backups) and (my $backup = $backups->next))
        {
            $backup->delete();
        }
        my $articles = $c->model('LizztCMSDB::LzArticle')->search({ article_id => $artid });
