...
package LizztCMS::Controller::Admin::Articles::JSON;
use Moose;
...
sub changeRevisionStatus
{
    ...
    my $art = $c->model('LizztCMSDB::LzArticle')->find({ article_id => $article, revision => $revision });
    if(not $art)
    {
    ...
    $c->model('LizztCMSDB::LzArticle')->search({
            article_id => $article,
            status_id => $status,
            revision => { '!=' => $revision },
...
sub ajaxHandler : Path('/admin/articles/ajax')
{
    ...
    elsif($wants eq 'folderList')
    {
        ...
        return $c->detach(qw(LizztCMS::Controller::Admin::Services folderList));
    }
    ...
    {
        $c->detach(qw(LizztCMS::Controller::Admin::Articles folderMoveCheck));
    }
...
sub fetchFileInfo
{
    ...
    {
        my $f = $c->model('LizztCMSDB::LzFile')->find({file_id => $file});
        $ret{$file} = $f->to_hash;
    }
...
sub fetchRevisionList
{
    ...
    my $article = $c->model('LizztCMSDB::LzArticle')->search({ article_id => $article_id} , { order_by => 'revision DESC' });
    my $return = [];
...
sub getArticleList : Private
{
    ...
    elsif(not my $folder_obj = $c->model('LizztCMSDB::LzFolder')->find({folder_id => $folder}))
    {
        return json_error($c,'INVALIDFOLDER');
    }
    my $articleList = $c->model('LizztCMSDB::LzArticleFolder')->search({folder_id => $folder},{ prefetch => 'article'});
...
sub assignFileToSpot : Local Path('/admin/articles/assignFileToSpot')
{
    ...
    $file = $c->model('LizztCMSDB::LzFile')->find({ file_id => $file });
    ...
    my $obj = $c->model('LizztCMSDB::LzArticleFile')->find({ article_id => $artid, file_id => $file->file_id});
    if(not $obj)
    {
