...
package LizztCMS::Controller::Admin::Files::Upload;
...
use LizztCMS::HelperModules::Includes qw(add_jsIncl add_cssIncl add_jsOnLoad);
use LizztCMS::HelperModules::Fields;
use LizztCMS::HelperModules::FileUploader qw(ERR_DIRNOTFOUND ERR_WRITEFAILURE);
use constant {
    TYPE_FILE        => 5,
    TYPE_TEMPLATE    => 6,
};
...
{
    my ( $self, $c ) = @_;
    my $classes = $c->model('LizztCMSDB::LzFileClass');
    if ($classes->count == 1)
    {
...
sub upload : Path Args(1) Form('/files/edit')
{
    ...
    if(not $c->model('LizztCMSDB::LzFileClass')->find({ id => $fileClassID }))
    {
        die; #FIXME
    }
    my $i18n = $c->stash->{i18n};
    my $fields = LizztCMS::HelperModules::Fields->new($c,'files',$fileClassID);
    $fields->editorInit();
    if ($c->req->param('asyncUpload'))
    {
        $c->stash->{onlySkeletonHTML} = 1;
    ...
    {
        my $data = $c->forward(qw(LizztCMS::Controller::Admin::Files::Edit get_forminfo), []);
        ...
        if(defined $c->req->param('artid'))
        {
            ...
            {
                my $art = $c->model('LizztCMSDB::LzArticle')->find({ article_id => $c->stash->{artid} });
                if ($art)
                {
        ...
        my $folders = $c->forward(qw(LizztCMS::Controller::Admin::Services buildtree), [ $defaultFolder, undef, 'write' ]);
    ...
    my $info = $c->forward(qw(LizztCMS::Controller::Admin::Files::Edit get_input_settings) ,[$form,'files']);
    for my $fno (1..$files)
    {
        foreach my $file ($c->req->upload('upload_file_no_'.$fno))
        {
            ...
            my $fields = LizztCMS::HelperModules::Fields->new($c,'files',$obj->file_id);
            $fields->saveData();
            $obj->update();
        }
    }
    ...
    {
        $c->detach(qw(LizztCMS::Controller::Admin::Files messageToList),[ $i18n->get('File(s) successfully uploaded') ]);
    }
}
...
sub handleData : Private
{
    ...
    my $fUploader = LizztCMS::HelperModules::FileUploader->new(c => $c);
...
{
    my ($self, $c, $form) = @_;
    my $fileObj = $c->model('LizztCMSDB::LzFile');
