...
package LizztCMS::Controller::Admin::Files::Edit;
...
sub default : Path('/admin/files/edit') Local Args
{
    ...
    {
        return $c->forward('LizztCMS::Controller::Admin::Files','messageToList',[$i18n->get('Error: Failed to locate UID. The path specified is invalid.')]);
    }
    my $file = $c->model('LizztCMSDB::LzFile')->find({file_id => $uid});
    if(not $file)
    {
        return $c->forward('LizztCMS::Controller::Admin::Files','messageToList',[$i18n->get('Error: That file does not exist.')]);
    }
    elsif(not $file->can_edit($c))
    {
        return $c->user->access_denied();
    }
    my $fields = LizztCMS::HelperModules::Fields->new($c,'files',$file->file_id, {
    ...
    {
        my $folders = $c->forward(qw(LizztCMS::Controller::Admin::Services buildtree), [ $f->folder_id, undef, 'write' ]);
...
sub get_forminfo : Private
{
    ...
    $c->stash->{classes} = $c->model('LizztCMSDB::LzFileClass');
...
sub savedata : Private
{
    ...
    {
        my $class = $c->model('LizztCMSDB::LzFileClass')->find({ id => $classID });
        if(not $class)
        {
            die; # FIXME
        }
    ...
    {
        return $c->forward('LizztCMS::Controller::Admin::Files','messageToList',[$i18n->get('File saved.')]);
    }
}
...
sub handleFolders : Private
{
    ...
    foreach my $folder (sort keys %{$c->req->params})
    {
        ...
        $c->model('LizztCMSDB::LzFileFolder')->create({
