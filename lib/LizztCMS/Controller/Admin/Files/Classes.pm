...
package LizztCMS::Controller::Admin::Files::Classes;
use 5.010;
use Moose;
use MooseX::NonMoose;
...
sub savedata
{
    ...
    {
        $fileClass = $c->model('LizztCMSDB::LzFileClass')->find({ id => $data{id} });
        $fileClass->set_column('name',$data{name});
        $fileClass->update();
    }
    else
    {
        $fileClass = $c->model('LizztCMSDB::LzFileClass')->create({ name => $data{name} });
        $fileClass->update();
...
sub edit : Args(1) Local
{
    my ($self, $c,$fileID) = @_;
    $self->checkSavedata($c);
    my $fileClass = $c->model('LizztCMSDB::LzFileClass')->find({ id => $fileID });
    if ($fileClass)
    {
        $c->stash->{values} = {
            id => $fileClass->id,
