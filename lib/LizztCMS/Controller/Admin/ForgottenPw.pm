...
package LizztCMS::Controller::Admin::ForgottenPw;
use Moose;
use MooseX::NonMoose;
...
sub pstpwd : Path('/admin/pstpwd')
{
    ...
    {
    	my $user_email = $c->model('LizztCMSDB::LzUser')->find({email => $c->req->param('user_email')});
	    if (not $user_email)
	    {
	        $msg_typ = 2;
	    }
	    else
	    {
            ...
            $unique_code =~ s/\//L/g;
    	    while($c->model('LizztCMSDB::LzUser')->find({reset_code => $unique_code}))
	        {
            ...
            my $message = $i18n->get_advanced("Please click on following link to change your LizztCMS password:\n %(LINK)",{ USERNAME => $db_user_name, LINK => $link});
    	    my $to =$db_email;
    ...
    $c->forward(qw(LizztCMS::Controller::Admin::ForgottenPw forgottenpw) , [ $msg_typ ]);
}
...
{	  
    ...
    my $obj_reset_code = $c->model('LizztCMSDB::LzUser')->find({reset_code => $get_reset_code});
...
sub chngpwd :  Path('/admin/chngpwd')
{
    ...
    {	
        ...
        {
            my $obj_rest_code = $c->model('LizztCMSDB::LzUser')->find({reset_code => $pst_reset_code});
