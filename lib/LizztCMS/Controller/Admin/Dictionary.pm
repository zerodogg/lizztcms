...
package LizztCMS::Controller::Admin::Dictionary;
...
sub index : Path Args(0) Form('/core/search') {
    ...
    my $dictionary = $c->model('LizztCMSDB::LzKeyValue')->search({
            type => 'dictionary'
        });
...
sub delete: Local Args
{
    ...
    {
        $definition = $c->model('LizztCMSDB::LzKeyValue')->find({keyvalue_id => $uid, type => 'dictionary'});
        $definition->delete();
...
sub add: Local Form('/dictionary/edit') {
    ...
    {
        my $exists = $c->model('LizztCMSDB::LzKeyValue')->find({thekey => $form->fields->{word}, type => 'dictionary'});
        if ($exists)
        {
...
sub edit: Local Args Form('/dictionary/edit') {
    ...
    else
    {
        ...
        else
        {
            ...
            my $dictionary = $c->model('LizztCMSDB::LzKeyValue')->find({keyvalue_id => $uid, type => 'dictionary'});
            if(not $dictionary)
            {
...
sub savedata: Private
{
    ...
    if ($type eq 'edit')
    {
        $category = $c->model('LizztCMSDB::LzKeyValue')->find({keyvalue_id => $uid});
    }
    elsif($type eq 'add')
    {
        $category = $c->model('LizztCMSDB::LzKeyValue')->create({ type => 'dictionary'});
    }
    else
    {
