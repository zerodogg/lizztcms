...
# This file contains various LizztCMS back-end services that doesn't really
# fit any of the others, or is too generic to be tied to a specific controller.
package LizztCMS::Controller::Admin::Services;
use Moose;
...
sub multiRequest : Local
{
    ...
    my %reqMap = (
        '/articles'                         => [ qw(LizztCMS::Controller::Admin::Articles index) ],
        '/articles/ajax'                    => [ qw(LizztCMS::Controller::Admin::Articles::JSON ajaxHandler) ],
        '/articles/submit'                  => [ qw(LizztCMS::Controller::Admin::Articles::JSON submit) ],
        '/articles/workflow/submit'         => [ qw(LizztCMS::Controller::Admin::Articles::Workflow submit) ],
        '/files'                            => [ qw(LizztCMS::Controller::Admin::Files index) ],
        '/services/templateList'            => [ qw(LizztCMS::Controller::Admin::Services templateList) ],
        '/services/folderList'              => [ qw(LizztCMS::Controller::Admin::Services folderList) ],
        '/services/templateInfo'            => [ qw(LizztCMS::Controller::Admin::Services templateInfo) ],
        '/services/filesInArticle'          => [ qw(LizztCMS::Controller::Admin::Services filesInArticle) ],
        '/services/roleAndUserList'         => [ qw(LizztCMS::Controller::Admin::Services roleAndUserList) ],
        '/services/permList'                => [ qw(LizztCMS::Controller::Admin::Services permList) ],
        '/articles/JSON/getTakenFileSpots'  => [ qw(LizztCMS::Controller::Admin::Articles::JSON getTakenFileSpots) ],
    );
...
{
    my ($self,$c) = @_;
    my $templates = $c->model('LizztCMSDB::LzTemplate')->search();
...
sub templateInfo : Local
{
    ...
    {
        $template = $c->model('LizztCMSDB::LzTemplate')->find({ type => 'article', is_default => 1});
    }

    if(not $template)
    {
        $template = $c->model('LizztCMSDB::LzTemplate')->find({ template_id => $template_id });
    }
...
sub filesInArticle : Local
{
    ...
    my $article = $c->model('LizztCMSDB::LzArticle')->find({article_id => $artid},{prefetch => 'workflow'});

    # Perform ACL check
    $c->forward('LizztCMS::Controller::Admin::Articles::Workflow','writecheck',[undef, $article]) or $c->detach();

    my $files = $c->model('LizztCMSDB::LzArticleFile')->search({article_id => $artid},{prefetch => 'file'});
...
sub permList : Local
{
    ...
    my $perms = $c->model('LizztCMSDB::LzPerms')->search({
            object_id => $objId,
            object_type => $objType,
        });
...
sub setPerm : Local
{
    ...
    if ($object eq 'folder')
    {
        my $folder = $c->model('LizztCMSDB::LzFolder')->find({
                folder_id => $objectId,
            });
        if(not $folder)
        {
        ...
        if ($override)
        {
            ...
            if (@{$children})
            {
                ...
                my $perms = $c->model('LizztCMSDB::LzPerms')->search($search);
                while(my $p = $perms->next)
                {
                    $p->delete();
                }
    ...
    my $pobj = $c->model('LizztCMSDB::LzPerms')->find_or_create($findSet);
...
sub writeBackup : Private
{
    ...
    my $object = $c->model('LizztCMSDB::LzBackup')->find({
    ...
    {
        $object = $c->model('LizztCMSDB::LzBackup')->create({
            user_id => $c->user->user_id,
            backup_source => $source,
            backup_string => $data,
            });
...
sub backup : Local
{
    ...
    {
        my $backup = $c->model('LizztCMSDB::LzBackup')->find({backup_id => $c->req->param('delete'), user_id => $c->user->user_id});
        if (not $backup)
        {
...
sub getBackupList : Private
{
    ...
    my $backups = $c->model('LizztCMSDB::LzBackup')->search({ user_id => $c->user->user_id });
...
sub elements : Local
{
    ...
    {
        my $elements = $c->model('LizztCMSDB::LzKeyValue');
        if ($c->req->param('_submitted_list_search') and $c->req->param('query'))
        {
            $elements = LizztCMS::HelperModules::Search::perform_search($elements,$c->req->param('query'), [ 'thekey','value'],undef,$c);
        }
        my $i18n = $c->stash->{i18n};
        if (not $elements)
        {
    ...
    elsif($action eq 'info')
    {
        ...
        my $element = $c->model('LizztCMSDB::LzKeyValue')->find({ keyvalue_id => $element_id });
    ...
    elsif($action eq 'save')
    {
        ...
        {
            $element = $c->model('LizztCMSDB::LzKeyValue')->find({ keyvalue_id => $element_id });
            if(not $element)
            {
                return json_error($c,'ELEMENTNOTFOUND');
            }
        ...
        {
            $element = $c->model('LizztCMSDB::LzKeyValue')->create({});
        }
...
sub buildtree : Private
{
    ...
    if(not $obj)
    {
        ...
        $obj = $c->model('LizztCMSDB::LzFolder')->search({ parent => \'IS NULL' });
    }
...
sub jsFilter: Local
{
    ...
    my $folders = $c->forward(qw(LizztCMS::Controller::Admin::Services buildtree),[undef,'array','read']);
    my $includeSearch = 0;
    my @filter;
    push(@filter,{
    ...
    my %SourceMap = (
        articles => 'LizztCMS::Controller::Admin::Articles',
        files => 'LizztCMS::Controller::Admin::Files',
        newsletter => 'LizztCMS::Controller::Admin::Newsletter',
    );
...
sub moveFolder: Local
{
    ...
    {
        my $parent = $c->model('LizztCMSDB::LzFolder')->find({ folder_id => $newParentID });
