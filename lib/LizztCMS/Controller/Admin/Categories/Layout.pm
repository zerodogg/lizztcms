...
package LizztCMS::Controller::Admin::Categories::Layout;
...
sub edit : Local Args
{
    ...
    {
        my $category = $c->model('LizztCMSDB::LzCategory')->find({category_id => $cat_id});
        if(not $category)
        {
...
sub renderCatArticleList : Local Args Form('/core/search')
{
    ...
    $category = $c->model('LizztCMSDB::LzCategory')->find({category_id => $cat_id});
    if (not $category)
    {
        return $self->messageToList($c,$i18n->get_advanced('Error: Failed to locate a category with the id %(CATEGORY_ID).', { CATEGORY_ID => $cat_id }));
    }
    if (defined (my $folder = $c->req->param('folder')))
    {
        ...
        {
             my $folder_cat = $c->model('LizztCMSDB::LzCategoryFolder')->find({category_id => $cat_id, folder_id => $folder});
             if (not $folder_cat)
             {
    ...
    my $result = $c->forward(qw( LizztCMS::Controller::Admin::Articles retrieveArticles ),[
            $c->model('LizztCMSDB::LzArticle'),
            $query,
            $self->formbuilder,
            0,
            $forceSearch
...
sub save : Local
{
    ...
    my $template = $c->model('LizztCMSDB::LzTemplate')->find({ template_id => $template_id });
    ...
    {
        my $previousOrdering = $c->model('LizztCMSDB::LzCategoryLayout')->search({ category_id => $category_id });
        $previousOrdering->delete;
        
        my $category = $c->model('LizztCMSDB::LzCategory')->find({ category_id => $category_id });
        ...
        for(my $spot = 0; $spot <= $totalSpots; $spot++)
        {
            ...
            $seen{$artid} = 1;
            my $insert_order_obj = $c->model('LizztCMSDB::LzCategoryLayout')->create({
