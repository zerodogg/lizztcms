...
package LizztCMS::Controller::Admin::Tags;
use Moose;
use namespace::autoclean;
BEGIN { extends 'Catalyst::Controller' };
use LizztCMS::HelperModules::JSON qw(json_response_raw json_response json_error);
...
sub complete : Local
{
    ...
    {
        my $results = $c->model('LizztCMSDB::LzTag')->search({ name => { 'LIKE' => $term.'%' } });
        if(not $results->count)
        {
            $results = $c->model('LizztCMSDB::LzTag')->search({ name => { 'LIKE' => '%'.$term.'%' } });
        }
...
sub create : Local
{
    ...
    my $newTag = $c->model('LizztCMSDB::LzTag')->create({ name => $term });
...
sub delete : Local
{
    ...
    my $tag = $c->model('LizztCMSDB::LzTag')->find({tag_id => $tagid});
    if(not $tag)
    {
        return json_error($c,'INVALID_TAG_ID');
    }
    my $artRels = $c->model('LizztCMSDB::LzArticleTag')->search({ tag_id => $tagid });
    while(my $rel = $artRels->next)
    {
        $rel->delete();
    }
    my $fileRels = $c->model('LizztCMSDB::LzFileTag')->search({ tag_id => $tagid });
...
{
    my($self,$c) = @_;
    my $tags = $c->model('LizztCMSDB::LzTag')->search(undef, { order_by => 'tag_id' });
    $c->stash->{tags} = $tags;
