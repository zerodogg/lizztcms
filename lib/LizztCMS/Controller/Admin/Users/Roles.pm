...
package LizztCMS::Controller::Admin::Users::Roles;
...
sub savedata : Private
{
    ...
    {
        $role = $c->model('LizztCMSDB::LzRole')->find({ role_id => $c->req->param('role_id')});
        if(not $role)
        {
    ...
    {
        $new = 1;
        $role = $c->model('LizztCMSDB::LzRole')->create({ });
    }
    ...
    # Run through actions
    my $classes = $c->model('LizztCMSDB::LzAction');
    while(my $action = $classes->next)
    {
        my $roleAction = $c->model('LizztCMSDB::LzRoleAction')->find_or_create({action_id => $action->action_id, role_id => $role->role_id});
    ...
    eval
    {
        my $actid = $c->model('LizztCMSDB::LzAction')->find({action_path => 'WORKFLOW_REASSIGN_TO_ROLE'})->action_id;
        my $roleAction = $c->model('LizztCMSDB::LzRoleAction')->find_or_create({action_id => $actid, role_id => $role->role_id});
    ...
    if($new)
    {
        my $aclEntry = $c->model('LizztCMSDB::LzAction')->find_or_create({action_path => 'WORKFLOW_REASSIGN_TO_ROLE_'.$role->role_id});
        if(not $aclEntry)
        {
...
sub edit : Local
{
    ...
    else
    {
        ...
        my $role = $c->model('LizztCMSDB::LzRole')->find( {'role_id' => $uid} );
        if(not $role)
        {
...
sub delete: Local
{
    ...
    {
        $role = $c->model('LizztCMSDB::LzRole')->find({role_id => $uid});
        if(not $role)
        {
        ...
        my $aclEntry = $c->model('LizztCMSDB::LzAction')->find({ action_path => 'WORKFLOW_REASSIGN_TO_ROLE_'.$uid });
        if ($aclEntry)
        {
            if(not $role)
            {
...
sub get_groups : Private
{
    ...
    my %aclGroups = (
        ...
        $i18n->get('Access essential LizztCMS services') => {
            paths => [
                '/services',
                '/services/jsFilter',
    ...
    my $customActions = $c->model('LizztCMSDB::LzAction')->search({
            -or => [
            {
                action_path => 
                {
