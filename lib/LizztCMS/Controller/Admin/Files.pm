...
package LizztCMS::Controller::Admin::Files;

use Moose;
use MooseX::NonMoose;
use namespace::autoclean;
BEGIN { extends 'Catalyst::Controller::FormBuilder' };
with 'LizztCMS::Role::List::Dual';
...
use LizztCMS::HelperModules::Search;
...
sub index : Path Args(0) Form('/core/search')
{
    ...
    else
    {
        ...
        my $dnd = LizztCMS::HelperModules::DragDrop->new($c,'LizztCMSDB::LzFolder','/admin/files/ajax/',
            {
                name => 'folder_name',
                uid => 'folder_id',
            },
...
{
    my ($self,$c) = @_;
    my $file = $c->model('LizztCMSDB::LzFile');
    if ($c->req->param('childOf'))
    {
        $file = $file->search({ clone => $c->req->param('childOf') });
        $c->stash->{childOf} = $c->req->param('childOf');
