...
package LizztCMS::Controller::Admin::Articles;
...
use LizztCMS::HelperModules::Forms qw(finalize_form);
use LizztCMS::HelperModules::DragDrop;
use LizztCMS::HelperModules::Lists qw(reply_json_list);
use LizztCMS::HelperModules::Includes qw(add_jsIncl add_cssIncl add_jsOnLoad add_globalJSVar add_jsOnLoadHeadCode);
use LizztCMS::HelperModules::Calendar qw(create_calendar datetime_to_SQL datetime_from_SQL datetime_from_unix);
use LizztCMS::HelperModules::Editor qw(create_editor);
use LizztCMS::HelperModules::JSON qw(json_response json_error);
use LizztCMS::HelperModules::Fields;
use LizztCMS::HelperModules::HTMLFilter qw(filter_string);
use LizztCMS::HelperModules::TemplateRenderer;
use LizztCMS::HelperModules::RevisionHelpers qw(article_latest_revisions get_latest_article set_other_articles_inactive);
...
sub index : Path Args(0) Form('/core/search')
{
    ...
    my $article = $c->model('LizztCMSDB::LzArticle');
                    ...
                    if ($articleobj->can_write($c))
                    {
                        if ((defined $assigned_by && $assigned_by == $c->user->user_id) || (defined $workflow->assigned_to_user && $workflow->assigned_to_user == $c->user->user_id) || $c->user->can_access('SUPER_USER'))
                        {
                            if ($updatedassignedto =~ s/^user_//)
                            {
                                ...
                                {
                                    my $userid = $c->model('LizztCMSDB::LzUser')->find({user_id => $updatedassignedto});
                            ...
                            {                                
                                if (not $updatedassignedto=~/\D/)
                                {
                                    my $roleid = $c->model('LizztCMSDB::LzRole')->find({role_id => $updatedassignedto});
    ...
    if(not $c->req->param('orderby'))
    {
        ...
        if ($c->model('LizztCMSDB::LzBackup')->search({user_id => $c->user->user_id})->count())
        {
    ...
    else
    {
        ...
        my $dnd = LizztCMS::HelperModules::DragDrop->new($c,'LizztCMSDB::LzFolder','/admin/articles/folderAjax/',
            {
                name => 'folder_name',
                uid => 'folder_id',
            },
...
sub init_searchFilters : Private
{
    ...
    my $userOptions = [];
    my $userOptionAccess = [];
    my $users = $c->model('LizztCMSDB::LzUser')->search(undef,{ order_by => 'user_name' });
    ...
    my $roles= $c->model('LizztCMSDB::LzRole')->search(undef,{ order_by => 'role_name' });
    ...
    my $statuses = $c->model('LizztCMSDB::LzStatus');
...
sub read : Local Args
{
    ...
    if ($revision)
    {
        $article = $c->model('LizztCMSDB::LzArticle')->find({ article_id => $uid, revision => $revision});
    }
    else
    {
    ...
    {  
        ...
        else
        {
            ...
            if ($article->files)
            {
                ...
                while(my $f = $file->next)
                {
                    ...
                    if ($fields)
                    {
                        ...
                        {
                            my $field = $c->model('LizztCMSDB::LzField')->find({ field_id => $f });
            ...
            my $fieldr = [];
            my $fields = LizztCMS::HelperModules::Fields->new($c,'articles',$article->article_id,{
                folder_id => $article->folder->folder_id,
            ...
            foreach my $field (@fieldList)
            {
                ...
                if ($field->inline)
                {
                    ...
                    my $val = $c->model('LizztCMSDB::LzArticle')->find({ article_id => $article->article_id, revision => $article->revision});
                    ...
                    elsif($fieldname eq 'template_id')
                    {
                        ...
                        if (defined $temp_id)
                        {
                            my $tmpl =  $c->model('LizztCMSDB::LzTemplate')->find({ template_id => $temp_id, type => 'article' });
                            $value = $tmpl->name;
                        }
                        else
                        {
            ...
            $c->stash->{artid} = $artid;
            $c->forward('LizztCMS::Controller::Admin::Articles::Workflow','preparePage',[ $article ]);
...
sub preview : Local Args
{
    ...
    if(not $template)
    {
        $template = $c->model('LizztCMSDB::LzTemplate')->find({ is_default => 1, type => 'article'});
    }
    my $renderer = LizztCMS::HelperModules::TemplateRenderer->new(
        c => $c,
        template => $template,
    );
    my $can_comment = 'false';
    if($c->forward('LizztCMS::Controller::Admin::Articles::Workflow','can_write',[undef, $article]) or $c->user->can_access('COMMENT_PREVIEWED_ARTICLES'))
    {
        $can_comment = 'true';
    }
    my $can_edit = 'false';
    ...
    my $includes = '<!-- LizztCMS preview includes -->'."\n";
    ...
    if ($c->stash->{lizztcmsLang})
    {
        push(@scripts,'/i18n/'.$c->stash->{lizztcmsLang}.'.js');
    }
    ...
    if ($c->stash->{lizztcmsLang})
    {
        $includes .= 'window._LANGUAGE = "'.$c->stash->{lizztcmsLang}.'";';
    }
    $includes .= '</script>';
    $includes .= "\n".'<!-- End of LizztCMS preview includes -->'."\n";
...
sub previewInfo : Private
{
    ...
    foreach my $field (@fieldList)
    {
        ...
        my $value = $c->model('LizztCMSDB::LzFieldValue')->find({ field_id => $field->field_id, module_name => 'articles', module_id => $article->article_id });
        next if not $value;
...
sub add: Local
{
    ...
    $c->forward('LizztCMS::Controller::Admin::Articles::Workflow','preparePage',[ undef ]);
}
...
sub edit: Local Args
{
    ...
    else
    {
        ...
        else
        {
            ...
            $c->forward('LizztCMS::Controller::Admin::Articles::Workflow','writecheck',[undef, $article]) or $c->detach();
...
sub delete: Local Args
{
    ...
    {
        my $backups = $c->model('LizztCMSDB::LzBackup')->search({ backup_source_id => $uid, backup_source => 'article'});
        while(( defined $backups) and (my $backup = $backups->next))
        {
            $backup->delete();
        }
        $article = $c->model('LizztCMSDB::LzArticle')->find({article_id => $uid});
...
sub buildform: Private
{
    ...
    my $fields = LizztCMS::HelperModules::Fields->new($c,'articles',$artid,{
            folder_id => $folder_id,
            revision => $revision,
    ...
    {
        my $folder = $c->model('LizztCMSDB::LzFolder')->find({ folder_id =>$folder_id});
    ...
    {
        if ($c->model('LizztCMSDB::LzFolder')->find({folder_id => $c->req->param('folder_id')}))
        {
...
sub art_fetch_fielddata
{
    ...
    {
        my @statusList;
        my $statuses = $c->model('LizztCMSDB::LzStatus')->search({});
        my $statusId = 1;
    ...
    elsif($fnam eq 'folder')
    {
        ...
        if (defined $folderId)
        {
            my $tree = $c->forward(qw(LizztCMS::Controller::Admin::Services buildtree),[ $folderId ]);
            if(not $folderId)
            {
    ...
    elsif($fnam eq 'template_id')
    {
        ...
        my $template = $c->model('LizztCMSDB::LzTemplate')->search({ type => 'article' }, { order_by => \'is_default,uniqueid' });
...
sub getCommentListFor : Local Args
{
    ...
    if (! ($article = $c->model('LizztCMSDB::LzArticle')->find({article_id => $articleId}, { prefetch => 'comments' })))
    {
...
sub deleteComment : Local Args
{
    ...
    my $comment;
    if(not ($comment = $c->model('LizztCMSDB::LzLiveComment')->find({comment_id => $commentId})))
    {
...
sub folderMoveCheck : Private
{
    ...
    my $article = $c->model('LizztCMSDB::LzArticle')->search({ article_id => $article_id }, { order_by => \'revision DESC' })->next;
    if (not $article)
    {
...
sub buildtree : Private
{
    ...
    shift;shift;
    return $c->forward(qw(LizztCMS::Controller::Admin::Services buildtree),\@_);
}
