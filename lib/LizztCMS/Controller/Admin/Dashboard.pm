...
package LizztCMS::Controller::Admin::Dashboard;

use strict;
use warnings;
use base 'Catalyst::Controller';
use LizztCMS::HelperModules::RevisionHelpers qw(article_latest_revisions);
use LizztCMS::HelperModules::Includes qw(add_jsIncl);
use LizztCMS::HelperModules::Widget;
...
{
    my ( $self, $c ) = @_;
    my $widget = LizztCMS::HelperModules::Widget->new($c,'MyAssignments');
    ...
    my $assignments;
    if ($assignments = $c->model('LizztCMSDB::LzWorkflow')->search({'me.assigned_to_user' => $c->user->user_id, 'article.trashed' => { '!=' => '1' }}))
    {
        $assignments = $assignments->search_related('article',{ -and => \@finalIgnore }, { order_by => 'modified_time DESC' });
    }
    my $statuses = $c->model('LizztCMSDB::LzStatus')->search({ -and => \@finalIgnore });
    my @includedStatuses;
...
{
    my ( $self, $c ) = @_;
    my $assignments = $c->model('LizztCMSDB::LzWorkflow')->search({assigned_to_role => $c->user->role->role_id, assigned_by => { '!=',$c->user->user_id}}, { rows => 30, order_by => 'deadline,start_date DESC'});
...
{
    my ( $self, $c, $status ) = @_;
    my $status_obj = $c->model('LizztCMSDB::LzStatus')->find({status_id => $status});
...
{
    my ( $self, $c ) = @_;
    my $dashboard_comments = $c->model('LizztCMSDB::LzLiveComment')->search(undef,{ order_by => 'created_date DESC', rows => 30 });
