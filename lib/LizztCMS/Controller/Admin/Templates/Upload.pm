...
package LizztCMS::Controller::Admin::Templates::Upload;
...
use LizztCMS::HelperModules::System qw(silentSystem);
use LizztCMS::HelperModules::Templates qw(parse_templatefile resolve_dependencies);
...
sub submitTemplate : Local
{
    ...
    my (undef, $targetFile) = tempfile('lz_templateUpload.XXXXXXXXX',
        DIR => $c->config->{LizztCMS}->{temp_path},
        SUFFIX => '.mas');
    ...
    eval
    {
        $template = $c->model('LizztCMSDB::LzTemplate')->create({
                type => $info->{TYPE},
    ...
    my $current = $c->model('LizztCMSDB::LzTemplate')->search({ is_default => 1, type => $template->type });
    if (!$current || !$current->count >0)
    {
        $template->set_column('is_default',1);
    }
    $template->update();
    move($targetFile,$c->config->{LizztCMS}->{template_path}.'/'.$fnam);
    $c->stash->{template} = 'adm/core/dummy.html';
...
sub validateTemplate : Private
{
    ...
    {
        $self->error($c,'APIVERSION not supported. This version of LizztCMS only supports APIVERSION 3 and lower, this template is for APIVERSION '.$data->{'APIVERSION'});
    }
...
#    if(my $r = silentSystem($LizztCMS::PATH.'/tools/masontest.pl',$file))
#    {
#        $self->error($c,'The file does not validate as well-formed mason/perl code. Please validate the template with some tool (such as ./tools/masontest.pl as included in LizztCMS) and fix the errors it reports before re-uploading.');
#    }
