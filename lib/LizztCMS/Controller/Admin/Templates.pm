...
package LizztCMS::Controller::Admin::Templates;
...
sub index : Path Args(0) Form('/core/search')
{
    ...
    my $template = $c->model('LizztCMSDB::LzTemplate');
...
sub setDefault : Local Param
{
    ...
    my $template = $c->model('LizztCMSDB::LzTemplate')->find({ template_id => $templateId });
