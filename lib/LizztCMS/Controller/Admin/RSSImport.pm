...
package LizztCMS::Controller::Admin::RSSImport;
...
use LizztCMS::HelperModules::Calendar qw(datetime_from_unix datetime_to_SQL);
...
sub index : Path Args(0) Form('/core/search')
{
    ...
    my $s = $c->model('LizztCMSDB::LzRssArticle');
...
sub importAllFeeds : Private
{
    ...
    foreach my $src (@{$c->config->{LizztCMS}->{rss_sources}})
    {
...
sub RSS_Delete : Private
{
    ...
    my $obj = $c->model('LizztCMSDB::LzRssArticle')->find({rss_id => $id});
...
sub RSSSubmitResponse : Private
{
    ...
    if ($id eq 'new')
    {
        my $guid = sha256_hex('lizztcms_manual_'.time().'_'.$c->user->user_id);
        $obj = $c->model('LizztCMSDB::LzRssArticle')->create({guid => $guid});
        $obj->set_column('pubdate',datetime_to_SQL(datetime_from_unix(time())));
    }
    else
    {
        $obj = $c->model('LizztCMSDB::LzRssArticle')->find({rss_id => $id});
    }
...
sub saveData : Private
{
    ...
    {
        my $item = $c->model('LizztCMSDB::LzRssArticle')->find({rss_id => $rssid});
        if(not $item)
        {
...
sub importFromFeed : Private
{
    ...
    my $m = $c->model('LizztCMSDB::LzRssArticle');
    ...
    foreach my $entry ($feed->get_item())
    {
        ...
        if ($c->config->{LizztCMS}->{rssImportActiveDefault})
        {
