...
package LizztCMS::Controller::Admin::Users;
...
{
    my ( $self, $c, $query ) = @_;
    my $users = $c->model('LizztCMSDB::LzUser');
...
sub init_searchFilters : Private
{
    ...
    my $roleOptions = [];
    my $roles= $c->model('LizztCMSDB::LzRole');
...
sub buildform: Private
{
    ...
    my $fieldObj = LizztCMS::HelperModules::Fields->new($c,'users',$user_id);
    $fieldObj->editorInit();
...
sub savedata: Private
{
    ...
    if ($type eq 'edit')
    {
        $user = $c->model('LizztCMSDB::LzUser')->find({user_id => $uid});
    }
    elsif($type eq 'add')
    {
        $user = $c->model('LizztCMSDB::LzUser')->new_result(
            {
                created => \'now()',
            }
        );
    ...
    if ($fields->{role})
    {
        my $role = $c->model('LizztCMSDB::LzRole')->find({role_name => $fields->{role}});
        if ($role)
        {
    ...
    my $fieldObj = LizztCMS::HelperModules::Fields->new($c,'users',$user->user_id);
    $fieldObj->saveData();
    if ($c->stash->{myAccountMode})
    {
...
sub add: Local Form('/users/edit') {
    ...
    {
        my $user_uid = $c->model('LizztCMSDB::LzUser')->find({user_id => $form->fields->{uid}});
        my $user_name= $c->model('LizztCMSDB::LzUser')->find({user_name => $form->fields->{user_name}});
        if ($user_name)
        {
...
sub edit: Args Local Form('/users/edit') {
    ...
    else
    {
        ...
        if (not defined $uid or $uid =~ /\D/) {
            ...
            my $user = $c->model('LizztCMSDB::LzUser')->find({user_id => $uid});
            if(not $user) {
...
sub redirWithError : Private
{
    ...
    if ($c->stash->{myAccountMode})
    {
        ...
        $c->detach(qw(LizztCMS::Controller::Admin::Users myaccount));
    }
    elsif(not defined $uid)
    {
        $c->stash->{message} = $errorAdd // $errorEdit;
        $c->detach(qw(LizztCMS::Controller::Admin::Users add), [ 1 ]);
    }
    else
    {
        $c->stash->{message} = $errorEdit;
        $c->detach(qw(LizztCMS::Controller::Admin::Users edit), [ $uid ]);
    }
}
...
sub myaccount : Path('/admin/myaccount')
{
    ...
    return $c->forward(qw(LizztCMS::Controller::Admin::Users edit) , [ $c->user->user_id, 1 ]);
}
