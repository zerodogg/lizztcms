...
package LizztCMS::Controller::Admin::Categories;
...
use LizztCMS::HelperModules::Search qw(cross);
use LizztCMS::HelperModules::Forms qw(finalize_form);
use LizztCMS::HelperModules::Includes qw(add_jsIncl add_cssIncl add_globalJSVar add_jsOnLoad);
...
{
    my ( $self, $c, $query ) = @_;
    my $category = $c->model('LizztCMSDB::LzCategory');
...
sub add: Local Form('/categories/edit')
{
    ...
    {
        my $category_uid = $c->model('LizztCMSDB::LzCategory')->find({category_id => $form->fields->{uid}});
        my $category_name= $c->model('LizztCMSDB::LzCategory')->find({category_name => $form->fields->{category_name}});
        if ($category_name)
        {
...
sub edit: Local Args Form('/categories/edit')
{
    ...
    else
    {
        ...
        else
        {
            ...
            my $category = $c->model('LizztCMSDB::LzCategory')->find({category_id => $uid});
            if(not $category)
            {
...
sub delete: Local Args
{
    ...
    {
        $category = $c->model('LizztCMSDB::LzCategory')->find({category_id => $uid});
        my $immediateChildren = $c->model('LizztCMSDB::LzCategory')->search({ parent => $uid});
...
sub savedata: Private
{
    ...
    {
        $category = $c->model('LizztCMSDB::LzCategory')->create({});
    }
    else
    {
    ...
    if(defined $fields->{parent})
    {
        ...
        if ($newValue)
        {
            $newValue = $c->model('LizztCMSDB::LzCategory')->find({category_id => $newValue });
            if ($newValue)
            {
    ...
    {
        if(my $f = $c->model('LizztCMSDB::LzCategoryFolder')->create({category_id => $category->category_id, folder_id => $folder_id}))
        {
            $f->update();
        }
        else
...
sub buildtree : Private
{
    ...
    if(not $obj)
    {
        ...
        $obj = $c->model('LizztCMSDB::LzCategory')->search({ parent => \'IS NULL'});
    }
