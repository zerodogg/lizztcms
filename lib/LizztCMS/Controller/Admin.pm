...
package LizztCMS::Controller::Admin;
use strict;
use warnings;
use 5.010;
...
sub initI18N : Private
{
    ...
    if ($lang ne 'en_US')
    {
        ($c->stash->{lizztcmsLang} = $lang) =~ s/_.+//;
    }
    # Create the i18n object, which all localizable text should be passed through
    $c->stash->{i18n} = LizztCMS::HelperModules::I18N->new('lizztcms',$lang,$c->path_to('i18n','locale')->stringify);
}
