...
package LizztCMS::Controller::Files::Get;

use strict;
use warnings;
use base qw(Catalyst::Controller::FormBuilder);
use LizztCMS::HelperModules::Files qw(lizztcms_serve_static_file lizztcms_serve_scalar_file);
use LizztCMS::HelperModules::Live::CAPTCHA qw(serve_captcha);
...
sub captcha : Path(/files/captcha)
{
    ...
    lizztcms_serve_scalar_file($c,$data,$mimetype);
}
...
sub default : Path(/files/get)
{
    ...
    else
    {
        ...
        {
            $file = $c->model('LizztCMSDB::LzFile')->find({ file_id=> $uid });
        }
    }
    ...
    lizztcms_serve_static_file($c,$filePath, $file->get_mimetype($c,$c->req->param('viewable')));
}
...
{
    my ( $self, $c ) = @_;
    my $renderer = LizztCMS::HelperModules::TemplateRenderer->new(c => $c);
    $renderer->error(404);
}
