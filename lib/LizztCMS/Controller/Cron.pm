...
sub default : Path('/cron')
{
    ...
    if ($trigger eq 'lizztcms_daily_cron')
    {
        $self->daily($c);
    }
    elsif($trigger eq 'lizztcms_daily_cron_two')
    {
        $self->twiceAday($c);
    }
...
sub fetchRSS : Private
{
    ...
    try
    {
        $c->forward(qw(LizztCMS::Controller::Admin::RSSImport importAllFeeds));
    }
    catch
    {
...
sub _newsletterDaily : Private
{
    ...
    my $newsletters = $c->model('LizztCMSDB::LzNewsletterSubscription')->search({validated => 1});
    ...
    my $mailer = LizztCMS::HelperModules::Mailer->new( c => $c, streaming => 1 );
    while((defined $sendThese) && (my $n = $sendThese->next))
    {
        my $renderer = LizztCMS::HelperModules::TemplateRenderer::EMail->new( c => $c, subscription => $n);
