package LizztCMS::Schema::LzArticle;
...
use LizztCMS::HelperModules::HTMLRenderer;
use LizztCMS::HelperModules::Calendar qw(datetime_from_SQL datetime_from_SQL_to_unix);
use LizztCMS::HelperModules::Cache qw(get_ckey CT_DEFAULT CT_24H CT_1H);
use LizztCMS::HelperModules::StringInsertFilter;
...
__PACKAGE__->belongs_to('status' => 'LizztCMS::Schema::LzStatus', 'status_id');
__PACKAGE__->belongs_to('lockTable' => 'LizztCMS::Schema::LzArticleLock', 'article_id');
__PACKAGE__->belongs_to('template','LizztCMS::Schema::LzTemplate', 'template_id');
__PACKAGE__->belongs_to('workflow' => 'LizztCMS::Schema::LzWorkflow',{
    'foreign.article_id' => 'self.article_id',
    'foreign.revision' => 'self.revision'
    });
__PACKAGE__->has_many(relationships => 'LizztCMS::Schema::LzArticleRelations', {
    'foreign.article_id' => 'self.article_id',
    'foreign.revision' => 'self.revision'
    });
__PACKAGE__->has_many(comments => 'LizztCMS::Schema::LzLiveComment',{
        'foreign.article_id' => 'self.article_id',
    },
    { cascade_delete => 0 },
    );
__PACKAGE__->has_many(files => 'LizztCMS::Schema::LzArticleFile',{
    'foreign.article_id' => 'self.article_id',
    'foreign.revision' => 'self.revision'
    });
__PACKAGE__->has_many(folders => 'LizztCMS::Schema::LzArticleFolder',{
    'foreign.article_id' => 'self.article_id',
    'foreign.revision' => 'self.revision'
    });
__PACKAGE__->has_many(additionalElements => 'LizztCMS::Schema::LzArticleElements',{
    'foreign.article_id' => 'self.article_id',
    'foreign.revision' => 'self.revision'
    });
__PACKAGE__->has_many(tags => 'LizztCMS::Schema::LzArticleTag',{
    'foreign.article_id' => 'self.article_id',
    'foreign.revision' => 'self.revision'
    });
__PACKAGE__->belongs_to('revisionMeta' => 'LizztCMS::Schema::LzRevision', {
...
__PACKAGE__->has_many('fields' => 'LizztCMS::Schema::LzFieldValue', {
...
__PACKAGE__->has_many(
    category_layout => 'LizztCMS::Schema::LzCategoryLayout',
    {'foreign.article_id' => 'self.article_id'},
    { cascade_delete => 0 },
);

use Moose;
with 'LizztCMS::Role::Serializable';
with 'LizztCMS::Role::URLGenerator';
with 'LizztCMS::Role::IndexTriggers';
with 'LizztCMS::Role::SchemaHelpers';
...
# This is in place of
# __PACKAGE__->belongs_to('folder' => 'LizztCMS::Schema::LzArticleFolder','article_id');
#
...
# This is in place of
# __PACKAGE__->has_many('secondary_folders' => 'LizztCMS::Schema::LzArticleFolder','article_id');
#
...
sub filteredBody
{
    ...
    my $filter = LizztCMS::HelperModules::StringInsertFilter->new( string => $body, insertStr => \@content, c => $c );
...
# Summary: Retrieve the body of an article, rendered with LizztCMS image
...
# which case LizztCMS will do a best case guess from the data defined in the
...
sub renderBody
{
    ...
    my $HTMLR = LizztCMS::HelperModules::HTMLRenderer->new(
...
sub clearCache
{
    ...
    my $cats = $c->model('LizztCMSDB::LzCategory')->search({}, { columns => [ 'category_id' ]});
...
sub getFieldRaw
{
    ...
    my $value = $c->model('LizztCMSDB::LzFieldValue')->find({
...
sub _get_best_category
{
    ...
    {
        return $c->model('LizztCMSDB::LzCategory')->find({ category_id => $id });
    }
...
sub _get_best_category_for_folder
{
    ...
    {
        if ($bestCategory = $c->model('LizztCMSDB::LzCategory')->find({ category_id => $b }))
        {
            return $bestCategory;
        }
    }
...
sub lock
{
    ...
    if(not $lock = $self->lockTable)
    {
        try
        {
            $lock = $c->model('LizztCMSDB::LzArticleLock')->create({ article_id => $self->article_id,
                    locked_by_user => $c->user->user_id,
                    updated_at => \'now()',
                });
        }
