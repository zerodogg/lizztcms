package LizztCMS::Schema::LzFile;
...
use LizztCMS::HelperModules::Files qw(get_new_aspect get_new_aspect_constrained fsize);
use File::MMagic::XS qw(:compat);
use URI::Escape qw(uri_escape);
use Math::Int2Base qw( int2base );
use Try::Tiny;

__PACKAGE__->belongs_to('ownerUser' => 'LizztCMS::Schema::LzUser', { 'foreign.user_id' => 'self.owner' });
__PACKAGE__->has_many(
    'clones' => 'LizztCMS::Schema::LzFile', { 'foreign.clone' => 'self.file_id' },
    { cascade_delete => 0 },
);
__PACKAGE__->has_many('folders' => 'LizztCMS::Schema::LzFileFolder', 'file_id');
__PACKAGE__->has_many('articles' => 'LizztCMS::Schema::LzArticleFile', 'file_id');
__PACKAGE__->has_many(tags => 'LizztCMS::Schema::LzFileTag',{
...
sub _serializeExtra
{
    # Extra fields or values to serialize. See LizztCMS::Role::Serializable for
    # the format.
...
# TEMPLATE METHODS
# ---
# These methods are meant for use within LizztCMS templates, to ease use of
# file spots
...
sub getFieldContents
{
    ...
    my $field = $c->model('LizztCMSDB::LzFieldValue')->find({ 
            field_id => $fieldId,
...
sub getAllFields
{
    my($self,$c) = @_;
    my $href = {};
    my $fields = $c->model('LizztCMSDB::LzFieldValue')->search({ module_id => $self->file_id, module_name => 'files' });
...
#   what LizztCMS can play), if ALL is true then it will also return true for audio
#   files that LizztCMS can't play.
...
sub get_path
{
    ...
    my $path = $c->config->{LizztCMS}->{file_path};
    $path .= '/'.$self->file_id;
    return $path;
}
...
sub get_caption
{
    ...
    if(defined $artid)
    {
        ...
        my $rel = $c->model('LizztCMSDB::LzArticleFile')->find($search);
...
{
    my($self,$c) = @_;
    my $tmpFile = $c->config->{LizztCMS}->{temp_path}.'/lizztcms_flvProgress-file_id-'.$self->file_id.'.log';
    return $tmpFile;
}
...
#   This will fork off the LizztCMS FLV converter (tools/flvConverter), which
...
sub create_FLV
{
    ...
    my $r = system($LizztCMS::PATH.'/tools/flvConverter',$self->get_path($c),$self->get_flv_path($c),$self->get_flv_tmpFile($c));
    ...
    if ($r != 0)
    {
        $c->log->warn('Running: "'.$LizztCMS::PATH.'/tools/flvConverter" "'.$self->get_path($c).'" "'.$self->get_flv_path($c).'" "'.$self->get_flv_tmpFile($c).'" for file '.$self->file_id.' failed - returned '.$r);
    }
    return;
}
