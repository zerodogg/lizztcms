package LizztCMS::Schema::LzArticleRelations;
...
LizztCMS::Schema::LzArticleRelations
...
__PACKAGE__->belongs_to('owner' => 'LizztCMS::Schema::LzArticle', { 'foreign.article_id' => 'self.article_id', 'foreign.revision' => 'self.article_id' });
__PACKAGE__->has_many(
    'related' => 'LizztCMS::Schema::LzArticle',
...
sub get_reverse_chronology
{
    my ($self,$c) = @_;
    croak('$c is missing') if not $c;
    my $searcher = $c->model('LizztCMSDB::LzArticleRelations')->find({ related_article_id => $self->article_id, relation_type => 'previous'});
    if(not $searcher)
    {
        return undef;
    }
...
sub _get_chronology_depth
{
    ...
    my $m = $c->model('LizztCMSDB::LzArticleRelations');
    ...
    {
        my $arts = get_live_articles_from($c->model('LizztCMSDB::LzArticle'));
