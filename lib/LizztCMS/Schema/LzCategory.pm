package LizztCMS::Schema::LzCategory;
...
__PACKAGE__->has_many(children => 'LizztCMS::Schema::LzCategory', {
    'foreign.parent' => 'self.category_id',
    });
__PACKAGE__->belongs_to('template_self' => 'LizztCMS::Schema::LzTemplate',{
...
use LizztCMS::HelperModules::Cache qw(get_ckey CT_1H);
use LizztCMS::HelperModules::Live::Articles qw(get_live_articles_from get_live_sql);
use Moose;
...
use LizztCMS::HelperModules::ProxiedResultSet;
...
sub orderedRS
{
    ...
    return LizztCMS::HelperModules::ProxiedResultSet->new(
            normalBuilder => sub
            {
                my $proxy = shift;
            ...
            orderedBuilder => sub
            {
                ...
                my $layout = $c->model('LizztCMSDB::LzCategoryLayout')->search({ 'category_id' => $self->category_id }, { order_by => 'spot' });
                if (!$layout->count)
                {
                    return;
                }
                ...
                my $result = $c->model('LizztCMSDB::LzArticle');
...
sub _fetchCategoryChildren_preSQL
{
    ...
    my $articles = $c->model('LizztCMSDB::LzArticle')->search($liveSearch,$options);
    if (ref($limit))
    {
...
{
    my($self,$c) = @_;
    my $children = $c->model('LizztCMSDB::LzCategory')->search({ '-or' => [ {root_parent => $self->category_id}, {parent => $self->category_id}]}, { columns => [ 'category_id']});
...
sub _getFoldersFromCategories
{
    ...
    my $search = $c->model('LizztCMSDB::LzCategoryFolder')->search({'-or' => \@folderSearch},{ columns => ['folder_id']});
...
__PACKAGE__->has_many(children => 'LizztCMS::Schema::LzCategory', { 'foreign.parent' => 'self.category_id' });
__PACKAGE__->has_many(folders => 'LizztCMS::Schema::LzCategoryFolder', 'category_id');
__PACKAGE__->belongs_to(parent => 'LizztCMS::Schema::LzCategory');

__PACKAGE__->has_many(layouts => 'LizztCMS::Schema::LzCategoryLayout', 'category_id');
1;
