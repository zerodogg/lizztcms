package LizztCMS::Schema::LzUser;
...
__PACKAGE__->belongs_to('role' => 'LizztCMS::Schema::LzRole', 'role_id');
...
sub _accessWrap
{
    ...
    if (!$self->{ACL})
    {
        $self->{ACL} = LizztCMS::HelperModules::AccessControl->new( c => $self->c, userId => $self->user_id, roleId => $self->role_id );
    }
    if(scalar(@_) == 2)
    {
        # ID-based
