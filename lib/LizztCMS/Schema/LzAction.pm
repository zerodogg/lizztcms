package LizztCMS::Schema::LzAction;
...
sub description
{
    ...
    else
    {
        if ($self->action_path =~ /^STATUSCHANGE_/)
        {
            ...
            $status = $c->model('LizztCMSDB::LzStatus')->find({status_id => $status});
            if ($status)
            {
        ...
        elsif($self->action_path =~ /^WORKFLOW_REASSIGN_TO_ROLE_/)
        {
            ...
            $role = $c->model('LizztCMSDB::LzRole')->find({role_id => $role});
            if ($role)
            {
