package LizztCMS::Schema::LzFolder;
...
__PACKAGE__->has_many(categoryfolders => 'LizztCMS::Schema::LzCategoryFolder', 'folder_id');
__PACKAGE__->has_many(children => 'LizztCMS::Schema::LzFolder', { 'foreign.parent' => 'self.folder_id' });
__PACKAGE__->belongs_to(parent => 'LizztCMS::Schema::LzFolder');

use Carp;
use LizztCMS::HelperModules::Fields;
use Moose;
with 'LizztCMS::Role::AccessControl::Model';
...
sub recursive_delete
{
    ...
    if(not defined $newParent)
    {
        ...
        {
            my $folders = $c->model('LizztCMSDB::LzFolder')->search({
                    parent => \'IS NULL',
                });
    ...
    my $articles = $c->model('LizztCMSDB::LzArticleFolder')->search({
            primary_folder => 1,
    ...
    my $files = $c->model('LizztCMSDB::LzFileFolder')->search({
            folder_id => $self->folder_id
        });
    ...
    $c->model('LizztCMSDB::LzFieldModule')->search({
            module    => 'folders',
...
sub fields
{
    ...
    my $fields = LizztCMS::HelperModules::Fields->new($c,'articles',$self->folder_id,
        {
            folder_id => $self->folder_id
        });
