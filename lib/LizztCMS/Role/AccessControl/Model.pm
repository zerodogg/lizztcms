...
package LizztCMS::Role::AccessControl::Model;

use Moose::Role;
with 'LizztCMS::Role::AccessControl';
with 'LizztCMS::Role::AccessControl::ObjCache';
with 'LizztCMS::Role::AccessControl::PermChecker';
use LizztCMS::HelperModules::AccessControl::Folders;
...
{
    my $self = shift;
    if ($self->isa('LizztCMS::Schema::LzFolder'))
    {
        return 'folder';
    }
}
...
sub _getAclObject
{
    ...
    if ($self->isa('LizztCMS::Schema::LzFolder'))
    {
        return LizztCMS::HelperModules::AccessControl::Folders->new(c => $self->c);
    }
    else
    {
...
LizztCMS::Role::AccessControl::Model - access control checks on a model object
