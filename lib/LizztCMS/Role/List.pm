...
sub getListHelper
{
    ...
    given($self->listType)
    {
        when('database')
        {
            return LizztCMS::HelperModules::List::Database->new(%{$options});
        }

        when('indexer')
        {
            return LizztCMS::HelperModules::List::Indexer->new(%{$options});
        }

        when('dual')
        {
            return LizztCMS::HelperModules::List::Dual->new(%{$options});
        }

        default
        {
            croak('LizztCMS::Role::List should not be consumed directly');
        }
    }
}
