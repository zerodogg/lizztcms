...
package LizztCMS::HelperModules::Indexer::Result;
use Moose;
use 5.010;
use Carp;
...
sub _constructCache
{
    ...
    {
        my $articles = $self->c->model('LizztCMSDB::LzArticle')->search({ '-or' => \@searches_articles});
    ...
    if (@searches_files)
    {
        my $files = $self->c->model('LizztCMSDB::LzFile')->search({ '-or' => \@searches_files});
