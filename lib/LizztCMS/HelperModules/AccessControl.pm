...
# LizztCMS::HelperModules::AccessControl
# 
...
package LizztCMS::HelperModules::AccessControl;
...
with 'LizztCMS::Role::AccessControl';
...
sub access_denied
{
    ...
    $self->deniedOnce(1);
    if ($c->debug && $ENV{LizztCMS_ACL_DEBUG})
    {
        cluck('Access to '.$self->last_denied.' denied');
    }
    $c->detach('LizztCMS::Controller::Admin::ACL','access_denied');
...
sub _singleCheck
{
    ...
    else
    {
        ...
        # Fetch the action id
        my $actionId = $c->model('LizztCMSDB::LzAction')->find({action_path => $check });
        if(not $actionId)
        {
            $c->log->error('Failed to look up LzAction object for '.$check.'. Denying access.');
            $return = false;
        }
        elsif(my $actionInfo = $c->model('LizztCMSDB::LzRoleAction')->find({role_id => $roleid, action_id => $actionId->action_id}))
        {
...
LizztCMS::HelperModules::AccessControl - access control checks for LizztCMS

=head1 DESCRIPTION

This module provides access to the access control checks contained in LizztCMS.
...
(LizztCMS path) supplied. Returns true if the user is permitted, false otherwise.
