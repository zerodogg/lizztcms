...
# (except use ->message()).
package LizztCMS::HelperModules::TemplateRenderer;
use Carp;
use Moose;
...
BEGIN
{
    ...
    no warnings;
    foreach my $module (glob($LizztCMS::PATH.'/lib/LizztCMS/HelperModules/TemplateRenderer/Resolver/*.pm'))
    {
        $module =~ s{^.*/([^/]+)\.pm$}{$1};
        load('LizztCMS::HelperModules::TemplateRenderer::Resolver::'.$module);
    }
};
...
sub resolve
{
    ...
    if(not defined $self->template)
    {
        ...
        if ($self->isa('LizztCMS::HelperModules::TemplateRenderer::URLHandler'))
        {
...
sub render
{
    ...
    $self->c->stash->{lizztcmsHeader} = '/core/liveHeader.html';
    ...
    $self->c->detach('LizztCMS::View::Mason');
}
...
sub message
{
    ...
    my $template = $self->c->model('LizztCMSDB::LzTemplate')->search({ type => 'message', is_default => 1 });
    my $fallback = 0;
    if(not $template->count)
    {
        $template = $self->c->model('LizztCMSDB::LzTemplate')->search({ type => 'message' });
    }
...
sub _get_resolver_for
{
    ...
    if ($type eq 'Article')
    {
        $resolver = LizztCMS::HelperModules::TemplateRenderer::Resolver::Articles->new($params);
    }
    else
    {
        $resolver = eval('return LizztCMS::HelperModules::TemplateRenderer::Resolver::'.$type.'->new($params)');
        my $e = $@;
        if(not $resolver)
        {
            my $helpful = '';
            ...
            die('Failed to dynamically locate resolver for type '.$type."\ntried: LizztCMS::HelperModules::TemplateRenderer::Resolver::$type\->new( resolver => $self, c => $self->c);\n".$helpful);
        }
    }
...
This is the LizztCMS template renderer. It handles retrieving information about
...
retrieval and rendering system that powers the live bits of LizztCMS sites.

=head1 ATTRIBUTES

All of these attribues can be supplied to the constructor
(LizztCMS::HelperModules::TemplateRenderer->new()) in a hash or hashref during
...
=head1 EXAMPLES
    ...
    my $handler = LizztCMS::HelperModules::TemplateRenderer->new(
        c => $c,
        template => $template
    );
