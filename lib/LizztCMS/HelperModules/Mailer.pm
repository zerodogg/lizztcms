package LizztCMS::HelperModules::Mailer;
use Moose;
...
#
# The guarding happens within the lizztcms-sendmail.pl process itself. If dupes
...
method add_mail($settings)
{
    ...
    {
        $footer //= $self->_i18n ?
            $self->_i18n->get('This message has been automatically generated by LizztCMS')
...
method _stream_out($data)
{
    ...
    if ( ! defined $socket)
    {
        $socket = '/tmp/.lizztcms-streaming-mailer-socket-'.$$.'-'.int(rand(999999999)).'-'.time.'-'.$<.'-'.$>.'-'.int(rand(999));
...
method _execute_mailer($data)
{
    ...
    my $pid = open2(my $out, my $in, lizztcmsFSPathTo('/tools/lizztcms-sendmail.pl')) or die("Failed to open2 to lizztcms-sendmail-pl\n");
...
method _version
{
    return lizztcmsVersion($self->c);
}

method _config()
{
    if ($self->c && $self->c->can('config') && $self->c->config && $self->c->config->{LizztCMS})
    {
        return $self->c->config->{LizztCMS};
    }
    return {};
}
