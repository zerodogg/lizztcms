...
# LizztCMS::HelperModules::DragDrop
#
...
# my $dnd = LizztCMS::HelperModules::DragDrop->new($c,'lzDBNAME','/path/tohandler/',
# {
...
package LizztCMS::HelperModules::DragDrop;
...
use LizztCMS::HelperModules::Includes qw(add_jsOnLoad add_jsHeadCode);
use LizztCMS::HelperModules::JSON qw(json_response json_error);

# Summary: Create a new drag and drop object
# Returns: New object
# Usage: $object = LizztCMS::HelperModules::DragDrop->new($c, MODELNAME, AJAXPATH, { fields }, { flags });
#   $c is the catalyst object
#   MODELNAME is the name of the model we're working with, ie. LizztCMSDB::LzSomething
...
sub new
{
    ...
    if(not $self->{modelName} =~ /^LizztCMSDB/)
    {
        $self->{c}->log->warn('DragDrop.pm ->new() got a modelName without a LizztCMSDB:: prefix, you may want to fix that: '.join(" ",caller));
    }
...
sub get_onload
{
    return [ 'lizztcms_DD_CreateDragDropEntities' ];
}
...
sub _handleCreate
{
    ...
    else
    {
        ...
        {
            my $field = $c->model('LizztCMSDB::LzField')->search({
                    inline => $_,
                });
            ...
            $field = $field->next;
            $c->model('LizztCMSDB::LzFieldModule')->create({
...
sub _buildXHTMLTree
{
    ...
    my $nodeNo = 0;
    my @RootObjs;
    if ($self->{modelName} eq 'LizztCMSDB::LzFolder')
    {
...
sub _get_JSHTML
{
    ...
    my $savedVal = $self->{c}->model('LizztCMSDB::LzUserConfig')->find({
    ...
    elsif($c->req->cookie('lizztcmsTreeView_expandedNodes'))
    {
        $confVal = $c->req->cookies->{'lizztcmsTreeView_expandedNodes'}->value;
    }
    else
    {
        $confVal = '';
    ...
    $ret .= "\t\t" . 'window.lizztcms_DD_URL = "'.$self->{ajaxHandler}.'";'."\n";
    ...
    $ret .= "\t\t" . 'lizztcms_DD_LastOrder = treeObj.getNodeOrders()'."\n";
    $ret .= "\t\t" . 'treeObj.orderChangeEvent = lizztcms_DD_OrderChangeEvent;'."\n";
    if ($confVal eq '')
    {
    ...
    if ($self->{flags}->{objClass})
    {
        $ret .= 'window.lizztcms_DD_FolderType = "multi"'."\n";
    }
    $ret .= "});"."\n";
    $ret .= '</script>'."\n";
    return $ret;
...
sub _saveExpanded
{
    ...
    my $entry = $self->{c}->model('LizztCMSDB::LzUserConfig')->find_or_create({
            user_id => $self->{c}->user->user_id,
            name => 'folderExpanded'
        });
