...
package LizztCMS::HelperModules::I18N;
use warnings;
use strict;
use POSIX;
...
# PATH is the local path to the directory contianing the locale tree,
# 	this can be undef, meaning that the module simply uses the default
# 	gettext paths.
sub new {
	my $Package = shift;
	my $self = {};
	...
	$self->{'localePath'} = shift;
	if ($self->{'hasLocale'})
	{
		$self->{'hasLocale'} .= '.UTF-8';
	}
	else
	{
		$self->{'hasLocale'} = '';
	...
	if(eval('use Locale::gettext;1')) {
		...
		if (defined $self->{'localePath'} and -d $self->{'localePath'})
        {
			if(my $I18N = $self->_getLocale())
			{
				if (-e $self->{'localePath'}.'/'.$I18N.'/LC_MESSAGES/'.$self->{'domain'}.'.mo')
				{
					$BindTo = $self->{'localePath'};
				}
			}
		}
...
{
    # LizztCMS: Needs the workaround to always be on, anything other than UTF-8 breaks horribly
    return 1;
