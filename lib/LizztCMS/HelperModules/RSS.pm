...
package LizztCMS::HelperModules::RSS;
use Moose;
use LizztCMS::HelperModules::Cache qw(get_ckey CT_24H CT_1H);
...
sub _lookupCategoryObjectFromName
{
    ...
    {
        $cat = $self->c->model('LizztCMSDB::LzCategory')->find({category_id => $cat});
    }
    ...
    if(not $cat)
    {
        ...
        $cat = $self->c->model('LizztCMSDB::LzCategory')->find({category_name => $name},{prefetch => 'children',columns => [ 'category_id','parent','root_parent']});
        ...
        if(not $cat)
        {
            ...
            if ($name =~ s/;+//g)
            {
                if ($cat = $self->c->model('LizztCMSDB::LzCategory')->find({category_name => $name},{prefetch => 'children',columns => [ 'category_id','parent','root_parent']}))
                {
                    $name =~ s/\s/-/g;
...
sub _fetchDefaultCategoryObject
{
    ...
    {
        return $self->c->model('LizztCMSDB::LzCategory')->find({ category_id => $self->defaultCatId });
    }
    return;
}
...
sub _getArticles
{
    ...
    {
        $articles = get_live_articles_from($self->c->model('LizztCMSDB::LzArticle'), { overrideLiveStatus => $self->overrideLiveStatus, extraLiveStatus => $self->extraLiveStatus});
