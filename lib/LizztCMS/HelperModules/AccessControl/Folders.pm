...
package LizztCMS::HelperModules::AccessControl::Folders;

# The basic user-facing methods can be found in ::PermBase. See that
# for the most commonly used API.

use Moose;
with 'LizztCMS::Role::AccessControl::ObjPerms';
...
sub get_perm_for_singleFolder
{
    ...
    my $perms = $self->c->model('LizztCMSDB::LzPerms')->search({
            object_type => 'folder',
...
LizztCMS::HelperModules::AccessControl::Folders - access control checks for LizztCMS folders

=head1 DESCRIPTION

This is a low-level module that provides access to access control checks for
LizztCMS folders.  This module is generally not called directly, it just
implements an interface that implements the back-end functions required for
with I<LizztCMS::Role::AccessControl::ObjPerms>. The methods provided in that role
