package LizztCMS::HelperModules::FileUploader;
use Moose;
use Graphics::Magick;
use constant {
    ERR_DIRNOTFOUND  => 1,
...
sub upload
{
    ...
    my $fileObj = $self->model('LizztCMSDB::LzFile');
    ...
    if (! defined $owner)
    {
        $owner = $self->model('LizztCMSDB::LzUser')->first;
    }
    ...
    if(not -d $self->c->config->{LizztCMS}->{file_path} or not -w $self->c->config->{LizztCMS}->{file_path})
    {
    ...
    if (!$self->standalone)
    {
        $self->c->forward(qw/LizztCMS::Controller::Admin::Files::Edit handleFolders/, [ $fileObj ]);
    }
