...
# LizztCMS::HelperModules::Fields
# 
# Object-oriented interface to LizztCMS fields
#
...
package LizztCMS::HelperModules::Fields;

use Moose;
use Carp;
use Scalar::Util qw(weaken);
use LizztCMS::HelperModules::Includes qw(add_jsIncl add_cssIncl add_bodyClass add_globalJSVar);
use LizztCMS::HelperModules::Calendar qw(datetime_to_SQL datetime_from_SQL datetime_from_SQL_to_unix);
use LizztCMS::HelperModules::Editor qw(create_editor);
use LizztCMS::HelperModules::Cache qw(get_ckey CT_DEFAULT);
use Carp;
...
# Usage: LizztCMS::HelperModules::Fields->new($c,$moduleName,$uid,$options);
# Parameters:
#   $c = catalyst object
...
sub _getValueColumn
{
    ...
    my $obj = $c->model('LizztCMSDB::LzField')->find({
            field_id => $field_id
        });
    if ($obj)
    {
...
sub addField
{
    ...
    {   
        ...
        $obj = $c->model('LizztCMSDB::LzFieldModule')->find_or_create({
                field_id => $field_id,
                module => $module,
    ...
    {   
        $obj = $c->model('LizztCMSDB::LzFieldModule')->find_or_create({field_id => $field_id, module => $self->module});
    }
    if ($obj)
    {   
        if(defined $position)
...
sub removeField
{
    ...
    {   
        $obj = $c->model('LizztCMSDB::LzFieldModule')->find_or_create({module => 'folders', object_id => $self->options->{folder_id}, field_id => $field_id});
    }
    elsif(defined $self->options->{object_id})
    {
        $obj = $c->model('LizztCMSDB::LzFieldModule')->find_or_create({ module => $self->module, field_id => $field_id, object_id => $self->options->{object_id} });
    }
    else
    {
        $obj = $c->model('LizztCMSDB::LzFieldModule')->find_or_create({module => $self->module, field_id => $field_id});
    }
    if (not $obj)
    {
...
sub editorInit
{
    ...
    if ($self->options->{additionalFields})
    {
        ...
        {
            my $f = $c->model('LizztCMSDB::LzField')->find({field_id => $field_id});
            if(not $f)
            {
...
sub saveData
{
    ...
    while(my ($field_id, $field_value) = each(%fields))
    {
        ...
        my $field = $c->model('LizztCMSDB::LzField')->find({field_id => $field_id});
        if(not $field)
        {
        ...
        if(my $current = $c->model('LizztCMSDB::LzFieldValue')->find($search))
        {
            my $val = { $self->_getValueColumn($c,$field_id) => $field_value };
            $self->_removeAndInsert($c->model('LizztCMSDB::LzFieldValue'), $search, $val);
        }
        else
        {
            ...
            my $m = $c->model('LizztCMSDB::LzFieldValue')->new_result({field_id => $field_id, module_name => $self->module, module_id => $self->uid, $self->_getValueColumn($c,$field_id) => $field_value});
...
sub get_fields
{
    ...
    else
    {
        ...
        if(defined $self->options->{object_id})
        {
            $fields = $c->model('LizztCMSDB::LzFieldModule')->search({module => $self->module, object_id => $self->options->{object_id}}, {order_by => 'position'});
        }
        else
        {
            $fields = $c->model('LizztCMSDB::LzFieldModule')->search({module => $self->module}, {order_by => 'position'});
        }
...
sub _prep_field
{
    ...
    elsif(my $fieldValue = $c->model('LizztCMSDB::LzFieldValue')->find({module_name => $self->module, module_id => $self->uid, field_id => $f->field_id, revision => $self->revision}))
    {
...
sub _get_fields_recursive
{
    my $self = shift;
    my $c = $self->c;
    my $folder = $c->model('LizztCMSDB::LzFolder')->find({folder_id => $self->options->{folder_id}});
    my $level = shift;
    $level = $level ? $level : 0;
    my %hasFields;
    my @finalFields;
    ...
    while($folder)
    {
        my $fields = $c->model('LizztCMSDB::LzFieldModule')->search({module => 'folders', object_id => $folder->folder_id});
