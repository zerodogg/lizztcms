...
# LizztCMS::HelperModules::Live::CAPTCHA
# 
package LizztCMS::HelperModules::Live::CAPTCHA;
...
sub get_captcha
{
    ...
    my $id = $c->model('LizztCMSDB::LzLiveCaptcha')->create({captcha => $captcha});
...
{
    my($c,$captcha_id) = @_;
    my $val = $c->model('LizztCMSDB::LzLiveCaptcha')->find({captcha_id => $captcha_id});
    if(not $val)
    {
        $val = 'ERROR (1)';
    }
    ...
    $magick->Annotate(
        text => $val,
        geometry => '+4+0',
        font => $LizztCMS::PATH.'/data/StayPuft.ttf',
        fill => 'black',
        gravity => 'West',
        #pointsize => 20,
        pointsize => 26,
...
sub validate_captcha
{
    ...
    my $capt = $c->model('LizztCMSDB::LzLiveCaptcha')->find({captcha_id => $captcha_id});
    if(not $capt)
    {
        return false;
    }
