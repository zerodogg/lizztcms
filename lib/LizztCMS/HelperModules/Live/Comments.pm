...
# LizztCMS::HelperModules::Live::Comments
# 
package LizztCMS::HelperModules::Live::Comments;
...
use LizztCMS::HelperModules::Live::CAPTCHA qw(validate_captcha get_captcha);
use LizztCMS::HelperModules::SimpleFormValidator qw(simple_validate_form);
...
sub comment_handle_input
{
    ...
    my $ip = $c->req->address;
    my $comment = $c->model('LizztCMSDB::LzLiveComment')->create({
            ip => $ip,
...
sub comment_prepare
{
    ...
    $c->stash->{$stashName.'comment_list'} = $c->model('LizztCMSDB::LzLiveComment')->search({article_id => $art_id}, { order_by => 'created_date' });
