...
package LizztCMS::HelperModules::Indexer;
use 5.010;
use Moose;
use Carp qw(carp croak);
use File::Path qw(mkpath);
use LizztCMS::HelperModules::Cache qw(get_ckey);
use LizztCMS::HelperModules::Indexer::Result;
...
with 'LizztCMS::Role::IndexerData';
...
{
    shift;
    return LizztCMS::HelperModules::Indexer::Result->new(@_);
}
...
{
    my $self = shift;
	if(not defined $self->c->config->{LizztCMS}->{indexer})
	{
		die('indexer not configured in the conf file');
	}
    return $self->c->config->{LizztCMS}->{indexer};
}
...
sub _isa
{
    ...
    if ($type eq 'article')
    {
        foreach my $isa (qw(LizztCMS::Model::LizztCMSDB::LzArticle LizztCMS::Schema::LzArticle))
        {
            if ($object->isa($isa))
            {
                return 1;
    ...
    {
        foreach my $isa (qw(LizztCMS::Model::LizztCMSDB::LzFile LizztCMS::Schema::LzFile))
        {
            if ($object->isa($isa))
            {
                return 1;
