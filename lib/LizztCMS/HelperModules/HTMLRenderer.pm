...
# with the templates.
package LizztCMS::HelperModules::HTMLRenderer;
use Moose;
use Mojo::DOM;
...
sub renderImg
{
    ...
    my $image = $self->c->model('LizztCMSDB::LzFile')->find({
    ...
    {
        $image = $self->c->model('LizztCMSDB::LzFile')->find({
...
sub _detectTemplate
{
    ...
    my $template = $self->c->model('LizztCMSDB::LzTemplate')->find({ type => 'media', is_default => 1 });
    if ($template)
    {
        return $template;
    }
    $template = $self->c->model('LizztCMSDB::LzTemplate')->search({ type => 'media' });
...
sub _template
{
    ...
    if (!@_)
    {
        ...
        {
            my $template = $self->c->model('LizztCMSDB::LzTemplate')->find({ uniqueid => $ret });
            $self->template($template);
            $ret = $template;
        }
    }
