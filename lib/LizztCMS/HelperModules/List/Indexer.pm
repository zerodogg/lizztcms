...
package LizztCMS::HelperModules::List::Indexer;
...
extends 'LizztCMS::HelperModules::List::Database';
...
sub _list_search_indexer
{
    ...
    my $indexer = LizztCMS::HelperModules::Indexer->new(
        c => $self->c,
    ...
    if ($self->objectName)
    {
        $self->c->stash->{ $self->objectName } = $result;
