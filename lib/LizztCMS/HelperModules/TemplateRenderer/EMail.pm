...
package LizztCMS::HelperModules::TemplateRenderer::EMail;
use Moose;
use Try::Tiny;
...
sub getRenderedContent
{
    ...
    my $renderer = HTML::Mason::Interp->new(
        ...
        data_dir => $self->c->config->{LizztCMS}->{temp_path},
        allow_globals => [ qw($c) ],
        out_method => \$output,
        preamble => 'use utf8;',
    );
...
sub autoSetTemplate
{
    ...
    my $template = $self->c->model('LizztCMSDB::LzTemplate')->find($search);
    if(not $template)
    {
...
sub autoGenerateArticleList
{
    ...
    my $articles = $self->c->model('LizztCMSDB::LzArticle')->search({ 'folders.folder_id' => { IN => $searchRef } }, { join => 'folders' });
