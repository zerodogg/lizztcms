...
package LizztCMS::HelperModules::TemplateRenderer::URLHandler;
use Moose;
use Try::Tiny;
use URI::Encode qw(uri_decode);
extends 'LizztCMS::HelperModules::TemplateRenderer';
...
sub _iHandleRequest
{
    ...
    if (defined($path[1]) && $path[1] =~ /^search(\?.*)?$/)
    {
        ...
        my $template = $self->c->model('LizztCMSDB::LzTemplate')->find({ is_default => 1, type => 'search' });
    ...
    elsif ($path[-1] =~ /^\d+(\?.+)?$/ || $path[-1] =~ /-\d+(\?.+)?$/)
    {
        ...
        {
            $template = $self->c->model('LizztCMSDB::LzTemplate')->find({ is_default => 1, type => 'article'});
        }
    ...
    elsif ($path[-1] eq '/' || $path[-1] =~ /\D/)
    {
        ...
        if ($path[-1] ne '/')
        {
            ...
            {
                $self->template( $self->c->model('LizztCMSDB::LzTemplate')->find({ type => 'list', is_default => 1}) );
            }
        }
        else
        {
            my $template = $self->c->model('LizztCMSDB::LzTemplate')->find({ type => 'list', is_default => 1 });
            $self->template($template);
...
sub getArticleFromURL
{
    ...
    my $artid = $path->[-1];
    $artid =~ s/^.*-(\d+)$/$1/;
    my $article = $self->c->model('LizztCMSDB::LzArticle')->find({ article_id => $artid, status_id => 2 });
    if ($article)
    {
    ...
    $article = $self->c->model('LizztCMSDB::LzArticle')->find({ article_id => $artid, 'revisionMeta.is_latest' => 1 }, { join => 'revisionMeta' });
    return ($article,undef);
}
...
sub getCategoryFromURL
{
    ...
    {
        return $self->c->model('LizztCMSDB::LzCategory')->find({ category_id => $catid });
    }
...
sub _resolveCategoryFromURL
{
    ...
    my $origCat = $path->[-1];
    my $cat     = $origCat;
    my $m       = $self->c->model('LizztCMSDB::LzCategory');
    # TODO: Caching
...
sub _tryFetchCategory
{
    ...
    my $list = $self->c->model('LizztCMSDB::LzCategory')->search({ 'me.category_name' => $name }, {prefetch => 'children'});
