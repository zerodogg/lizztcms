...
package LizztCMS::HelperModules::TemplateRenderer::Resolver::Files;
use Moose;
...
sub get_fileSpots
{
    ...
    if(my $data = $self->c->cache->get($ckey))
    {
        ...
        {
            $data->{$key} = $self->c->model('LizztCMSDB::LzArticleFile')->find({ article_id => $artid, file_id => $data->{$key}, revision => $article->revision});
        }
        return $data;
    }
