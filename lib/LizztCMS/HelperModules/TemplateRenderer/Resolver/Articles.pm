...
package LizztCMS::HelperModules::TemplateRenderer::Resolver::Articles;
use Moose;
with 'LizztCMS::Role::TemplateRenderer::Resolver';

use LizztCMS::HelperModules::Live::Comments qw(comment_handler comment_prepare);
    ...
    else
    {
        ...
        if ($searchContent->{catid})
        {
            ...
            if ($catid eq 'url' || $catid eq 'arturl')
            {
                ...
                {
                    $cat = $self->c->model('LizztCMSDB::LzCategory')->find({ category_id => $searchContent->{root_catid} });
            ...
            {
                $cat = $self->c->model('LizztCMSDB::LzCategory')->find({category_id => $catid},{prefetch => 'children' });
            }
            if ($cat)
            {
        ...
        {
              $obj = get_live_articles_from($self->c->model('LizztCMSDB::LzArticle'), { rows => $limit, order_by => 'publish_time DESC', extraLiveStatus => $searchContent->{extraLiveStatus}, overrideLiveStatus => $searchContent->{overrideLiveStatus} });
        }
...
sub get_article
{
    ...
    else
    {
        if ($searchContent->{artid})
        {
            ...
            {
                $obj = get_live_articles_from($self->c->model('LizztCMSDB::LzArticle'), { extraLiveStatus => $searchContent->{extraLiveStatus}, overrideLiveStatus => $searchContent->{overrideLiveStatus} });
...
sub _getCategoryObjectFromURL
{
    ...
    {   
        $isFrontPage = 1;
        $cat = $self->c->model('LizztCMSDB::LzCategory')->find({category_id => $settings->{root_catid}});
    }
    else
    {   
        my $origCat = $catname;
        $catname =~ s/-/ /g;
        $cat = $self->c->model('LizztCMSDB::LzCategory')->find({category_name => $catname},{prefetch => 'children'});
        # FIXME
        $self->c->stash->{rssCategory} = $origCat;
        if (not $cat)
        {   
            $cat = $self->c->model('LizztCMSDB::LzCategory')->find({category_name => $origCat},{prefetch => 'children'});
        }
    }
    if(wantarray())
    {
