...
package LizztCMS::HelperModules::TemplateRenderer::Resolver::Comment;
use Moose;
...
sub get_list
{
    ...
    my $res = $self->c->model('LizztCMSDB::LzLiveComment')->search(undef,{ rows => $limit, order_by => 'comment_id DESC'});
    return { $saveAs => $res };
}
