...
package LizztCMS::HelperModules::TemplateRenderer::Resolver::Category;
use Moose;
...
sub get_category
{
    ...
    if (!$category)
    {
        $self->log('Resolver Category get: Unable to complete request: no category statevar, this could be a bug in LizztCMS');
        return;
    }
    return { $saveAs => $c->model('LizztCMSDB::LzCategory')->find({ category_id => $category->category_id }) };
}
