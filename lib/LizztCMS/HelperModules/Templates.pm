...
# LizztCMS::HelperModules::Templates

package LizztCMS::HelperModules::Templates;

use strict;
use warnings;
use Carp;
use Exporter qw(import);
use LizztCMS::HelperModules::Cache qw(get_ckey);
...
# Returns an empty hashref if no LizztCMS_INFOBLOCK is found, OR if it was never ENDed
# Returns a hash of key=value pairs if a LizztCMS_INFOBLOCK was found and parsed
sub parse_templatefile
{
    ...
    while(<$i>)
    {
        ...
        if (not $started)
        {
            if (/^\s*BEGIN\s+LizztCMS_INFOBLOCK\s*$/)
            {
                $started = true;
            }
            next;
        }
        if (/^\s*END\s+LizztCMS_INFOBLOCK\s*$/)
        {
            $ended = true;
            last;
        }
    ...
    if(not $ended)
    {
        $c->log->warn($file.': found no END LizztCMS_INFOBLOCK. Discarding data');
        return {};
    }
    else
    {
...
sub resolve_dependencies
{
    ...
    {
        my $dbEntry = $c->model('LizztCMSDB::LzTemplate')->search({uniqueid => $template});
    ...
    while(@processing)
    {
        ...
        my $dbEntry = $c->model('LizztCMSDB::LzTemplate')->search({uniqueid => $curr});
...
sub get_parsed_template_info
{
    ...
    if(not -e $template)
    {
        ...
        {
            my $tObj = $c->model('LizztCMSDB::LzTemplate')->search({ uniqueid => $template});
            if ($tObj && $tObj->count)
            {
    ...
    else
    {
        ...
        $template = $c->model('LizztCMSDB::LzTemplate')->find({ file => basename($template) });
        if(not $template)
        {
