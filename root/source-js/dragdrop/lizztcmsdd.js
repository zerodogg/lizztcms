 ...
 */
/*
 * LizztCMS functions for drag+drop
 *
 ...
 * All Rights Reserved
 */
var lizztcms_DD_LastOrder;
var lizztcms_DD_URL;
var lizztcms_DD_FolderType = 'single';
var lizztcms_DD_DialogBox;
var folderLimit_override;
var lizztcms_DD_myDragDrop;

/*
 * Called whenever the ordering of the folder tree is changed
 */
function lizztcms_DD_OrderChangeEvent ()
{
    ...
    if(order == lizztcms_DD_LastOrder)
    {
        return;
    }
    lizztcms_DD_LastOrder = order;
    $.get(lizztcms_DD_URL+'?orderChange='+order,lizztcms_DD_OrderReply);
}

/*
 * The reply from the server concerning the order change
 */
function lizztcms_DD_OrderReply (text)
{
    text = text.split("\n");
 ...
 */
function lizztcms_DD_RefreshList ()
{
    $.get(lizztcms_DD_URL+'?request=HTML_LIST',lizztcms_DD_RefreshListReply);
}

/*
 * New folder tree recieved, rebuild using the new data
 */
function lizztcms_DD_RefreshListReply (text)
{
    text = text.split("\n");
    var reply = text.shift();
    destroyPI();
 ...
 */
function lizztcms_DD_ItemHide(item,parentItem)
{
 ...
 */
function lizztcms_DD_ItemDropped(sourceId, targetId, mouseX, mouseY)
{
    var sourceObj = document.getElementById(sourceId);
    lizztcms_DD_ItemHide(sourceObj,sourceObj.parentNode);
    ...
    $.get(lizztcms_DD_URL+'?moveToFolder='+target+'&item='+source,lizztcms_DD_ItemDropReply);
    return true;
}
 ...
 */
function lizztcms_DD_ItemDropReply(text)
{
    text = text.split("\n");
    var reply = text.shift();
    if(! reply.match(/^OK/))
...
function lizztcms_DD_CreateDragDropEntities()
{
    var sources = [];
    var targets = [];
    ...
    else
    {
        try
        {
            ...
            {
                dragdrop.addTarget(targets[i],'lizztcms_DD_ItemDropped');
            }
            dragdrop.init();
            lizztcms_DD_myDragDrop = dragdrop;
        }
        catch(e)
        {
            lzException(e);
 ...
 */
function lizztcms_DD_NewItem(junk,parentObj)
{
    ...
    XHR.GET('/admin/services/folderList?showRoot=true&selected='+encodeURIComponent(myparent),lizztcms_DD_NewFolderItem,null);
    return false;
}
/*
 * Shows the 'new folder' dialog
 */
function lizztcms_DD_NewFolderItem(form)
{
    destroyPI();
    ...
    {
        lizztcms_DD_CreateNewFolder();
        lizztcms_DD_DialogBox.destroy();
    };
    try
    {
        if(lizztcms_DD_DialogBox && lizztcms_DD_DialogBox.hide)
        {
            lizztcms_DD_DialogBox.hide();
        }
        lizztcms_DD_DialogBox = new dialogBox(html,{title: i18n.get('New folder'), buttons: buttons, minWidth: 500});
    }
    catch(e)
    {
        lzException(e);
 ...
 */
function lizztcms_DD_CreateNewFolder()
{
    var name;
    var myparent;
    try
    ...
    XHR.GET(lizztcms_DD_URL+'?parent='+encodeURIComponent(myparent)+'&addName='+encodeURIComponent(name), lizztcms_DD_NewItemSuccess, lizztcms_DD_NewItemFailure);
}
/*
 * All good, new folder created, so we kick off a refresh of the data
 */
function lizztcms_DD_NewItemSuccess (reply)
{
    lizztcms_DD_RefreshList();
}
/*
 ...
 */
function lizztcms_DD_NewItemFailure (reply)
{
