...
This is the LizztCMS template manual for web templates.
...
You can put site-specific controllers into either lib/LizztCMS/Controller/SiteHacks.pm
or anything under lib/LizztCMS/Controller/SiteHacks/. These will be preserved upon upgrade,
and allows you to add any controller you need into a LizztCMS instance, the former
...
what kind of pages and data LizztCMS will use it for. The allowed settings
are:
...
A template that is to be included in other templates. LizztCMS will
never use it directly.

=item message

A template that displays either a message from lizztcms, or an error
(either a standard error form LizztCMS, or a customized error message).
...
It MIGHT recieve a $lz_message_intMsg - which is an internal LizztCMS
...
A template that displays data from the LizztCMS RSS Import module.
This is quite similar to list, except that it gets data in a slightly
different format (and is not used by LizztCMS directly right now, it needs
...
by LizztCMS, and is the means that other templates have for referreing to
LizztCMS templates it wants to include. It can contain any string (without
...
MYDOMAIN is your domain name (for instance "example" or "lizztcms") and
...
template will include (using $m->comp() or <& &>). LizztCMS will resolve
...
will most likely not work in LizztCMS anyway, as the filenames are dynamically
...
template, it simply provides LizztCMS with information that it can use
...
can have. This is how you tell LizztCMS what kind of data your template wants.
LizztCMS will then look up the data and prepare it for use for you. The various
...
NEEDSINFO= entries, LizztCMS does not care which one you use (or even if
you mix the two).
...
you can use %20 and LizztCMS will convert it for you before retrieving the
data.
...
Will make LizztCMS consider 5 to be the live status_id, rather than whatever the id
...
either be a LizztCMS category number (int) or the string "url" (without the
quotes). If it is "url" or "arturl" then LizztCMS will fetch the article from the
...
If you omit this parameter then LizztCMS will fetch ALL articles that match
the other parameters, regardless of category. Any article that is live will
be returned.

=item B<root_catid>

This sets the 'root' category ID. This is the category ID that LizztCMS
...
value LizztCMS will not automatically go to a 404 error if the query failed
to return anything.
...
This determines the article ID of the article. It must either be a LizztCMS
article number (int) or the string "url" (without the quotes). If it is "url"
then LizztCMS will fetch the article from the URL (this is probably the most
common use for this option in article templates).

If you do not provide it then a 404 will be displayed instead, because LizztCMS
does not know which article to fetch.

=item B<handleComments>

This is a numeric boolean (1=true, 0=false). If it is set to any nonzero
value and the article has live comments enabled, LizztCMS will handle comment
...
and the article has live comments enabled, LizztCMS will retrieve any comments
...
This is a numeric boolean (1=true, 0=false). If it is set to any nonzero
value LizztCMS will retrieve relationships the article has. These will be
...
By default LizztCMS will order the results by index, with a tiny bias towards
...
comment list as $comment_list.  If LizztCMS recieves a submitted comment for the
...
as $lastComments. The resultSet contains I<LizztCMS::Schema::LzComment>s.

=item B<RSSImport_list_[limit=10,as=imported]>

This will fetch a list of the ten most recently imported RSS entries as
$imported. The resultSet contains I<LizztCMS::Schema::LzRssArticle>s.
...
LizztCMS contains builtin support for various features, such as video.
...
extra javascript files automatically using DOM objects. The core LizztCMS
JS also contains some helper functions that you may use inside your templates.
See the file I<root/source-js/live/core.js> within the LizztCMS tree.

Note that this includes jQuery (version will vary with each LizztCMS release,
